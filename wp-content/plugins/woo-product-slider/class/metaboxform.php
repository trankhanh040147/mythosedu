...
class SP_WPS_MetaBoxForm {
	...
	public function select( array $args ) {
		...
		echo $this->field_before( $args );
		echo sprintf(
			'<select name="%1$s" id="%2$s" class="sp-wps-input-text sp-wps-select" %3$s>', $name, $args['id'],
			$multiple
		);
	...
	public function layout( array $args ) {
		...
		echo $this->field_before( $args );
		echo '<div class="sp-field-image-select">';
		foreach ( $args['options'] as $key => $option ) {
			$selected = ( $args['default'] == $key ) ? ' checked="checked"' : '';
			echo '<label><input type="radio" name="' . $key . '" value="' . $key . '"' . $selected . '/><img src="' . $option . '" alt="' . $key . '" /><p><b>' . $key . '</b></p><span class="sp-wcsp-layout-before"><i class="fa fa-check" aria-hidden="true"></i></span></label>';
		}
		echo '</div>';
		echo $this->field_after();
	}
	...
	public function select_slider_type( array $args ) {
		...
		<option value="product_slider">Product</option>
		<option value="category_slider" disabled>Category (Pro)</option>
		<?php
		echo '</select>';
		echo $this->field_after();
	}
	...
	public function select_products_from( array $args ) {
		...
		echo $this->field_before( $args );
		echo sprintf(
			'<select name="%1$s" id="%2$s" class="sp-wps-input-text sp-wps-select wps_products_from" %3$s>', $name, $args['id'],
			$multiple
		);
		?>
		<option value="latest">Latest</option>
		<option value="featured" disabled>Featured (Pro)</option>
		<option value="products_from_category" disabled>Category (Pro)</option>
		<option value="products_from_tag" disabled>Tag (Pro)</option>
		<option value="best_selling" disabled>Best Selling (Pro)</option>
		<option value="related_products" disabled>Related (Pro)</option>
		...
		<option value="specific_products" disabled>Specific (Pro)</option>
		<option value="most_viewed" disabled>Most Viewed (Pro)</option>
		<option value="recently_viewed" disabled>Recently Viewed (Pro)</option>
		<option value="products_from_sku" disabled>SKU (Pro)</option>
		<option value="products_from_attribute" disabled>Attribute (Pro)</option>
		<option value="free_products" disabled>Free (Pro)</option>
		<option value="products_from_exclude_category" disabled>Exclude Category (Pro)</option>
		<option value="products_from_exclude_tag" disabled>Exclude Tag (Pro)</option>
		<?php
		echo '</select>';
		echo $this->field_after();
	}
