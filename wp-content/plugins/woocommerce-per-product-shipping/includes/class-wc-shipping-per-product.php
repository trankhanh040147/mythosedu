...
class WC_Shipping_Per_Product extends WC_Shipping_Method {
	...
	private function calculate_product_shipping_cost( array $product_data, array $package ) {
		...
		// Fee.
		$item_shipping_cost += $this->get_fee( (float) $this->fee, $item_shipping_cost );
			...
			foreach ( $package['contents'] as $item_id => $values ) {
				if ( $values['quantity'] > 0 ) {
					if ( $values['data']->needs_shipping() ) {
						if( in_array($values['product_id'], $_cartProductCalculated ) )
							continue;
						$_cartProductCalculated[] = $values['product_id'];
