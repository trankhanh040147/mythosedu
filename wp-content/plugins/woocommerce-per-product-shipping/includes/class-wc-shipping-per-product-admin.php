...
class WC_Shipping_Per_Product_Admin {
	 ...
	 */
	protected $per_product_init;

	/**
	 * Constructor.
	 *
	 * @param WC_Shipping_Per_Product_Init $per_product_init Referance to init object.
	 */
	public function __construct( WC_Shipping_Per_Product_Init $per_product_init ) {
		$this->per_product_init = $per_product_init;
	...
	public function output_rules( $post_id = 0 ) {
		...
		<div class="rules per_product_shipping_rules">
			...
			<?php
			if ( false !== $this->per_product_init->use_legacy_shipping_method() ) {
