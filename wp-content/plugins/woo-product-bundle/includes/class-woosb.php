...
if ( ! class_exists( 'WPCleverWoosb' ) && class_exists( 'WC_Product' ) ) {
	class WPCleverWoosb {
		...
		function __construct() {
			...
			// Cart item
			add_filter( 'woocommerce_cart_item_name', array( $this, 'woosb_cart_item_name' ), 10, 3 );
			...
			add_filter( 'woocommerce_order_item_name', array( $this, 'woosb_cart_item_name' ), 10, 3 );
		...
		function woosb_cart_item_name( $name, $cart_item, $cart_item_key ) {
		...
		function woosb_add_to_cart_items( $items, $cart_item_key, $product_id, $quantity ) {
			...
			if ( is_array( $items ) && ( count( $items ) > 0 ) ) {
				foreach ( $items as $item ) {
					...
					$_variation_id = 0;
					$_variation    = array();
					$_price        = WPCleverWoosb_Helper::woosb_get_price( $_product, 'min', $discount );
					...
					if( $_product instanceof WC_Product_Variable){
						$_variation_id = isset($item['variation_id']) ? $item['variation_id'] : 0;
						$_variation    = isset($item['variation_attribute']) ? $item['variation_attribute'] : '';
					}
			...
			if ( $product->is_type( 'woosb' ) && ( $ids = $product->get_ids() ) ) {
				...
				foreach ( $items as $item ) {
					...
					if($_product->is_type( 'variable' )){
						$get_variations = count( $_product->get_children() ) <= apply_filters( 'woocommerce_ajax_variation_threshold', 30, $_product );
						$available_variations = $get_variations ? $_product->get_available_variations() : false;
						$array_variation[$_id] = $available_variations;
					    echo sprintf('<input autocomplete="off" class="woosb_variation_id woosb_variation_id_%s" name="variation_id_%s" type="hidden" value="" />', $_id, $_id);
					    echo sprintf('<input autocomplete="off" class="woosb_variation_attribute woosb_variation_attribute_id_%s" name="variation_attribute_%s" type="hidden" value="" />', $_id, $_id);
                    }
				}
				wp_localize_script('wc-add-to-cart-variation', 'CT_WOOSB', $array_variation);
				...
				if ( ! $product->is_fixed_price() ) {
					...
					if ( $product->is_optional() ) {
						...
						foreach ( $items as $item ) {
							...
							if ( $_product ) {
								$prices[] = WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, 1, 'min', $discount );
							}
						}
						...
						foreach ( $items as $item ) {
							...
							if ( $_product ) {
								$min_price += WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, $item['qty'], 'min', $discount );
								$max_price += WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, $item['qty'], 'max', $discount );
							}
						}
						...
						foreach ( $items as $item ) {
							...
							if ( $_product ) {
								$_price = WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, $item['qty'], 'min', $discount );
                ...
                <div class="woosb-products"
					...
					foreach ( $items as $item ) {
                        ...
                        <div class="woosb-product"
                             ...
                             data-id="<?php echo esc_attr( $_product->is_type( 'variable' ) ? $item['id'] : $item['id'] ); ?>"
                             data-price="<?php echo esc_attr( WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, 1, 'min', $discount ) ); ?>"
                                ...
                                <div class="woosb-price">
                                    <div class="woosb-price-ori">
										...
										switch ( get_option( '_woosb_bundled_price', 'price' ) ) {
											case 'price':
												if ( $show_new_price ) {
													$_price = wc_format_sale_price( WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, 1, 'min', $discount ), wc_get_price_to_display( $_product, array( 'price' => $_product_price ) ) );
												} else {
											...
											case 'subtotal':
												if ( $show_new_price ) {
													$_price = wc_format_sale_price( WPCleverWoosb_Helper::woosb_get_price_to_display( $_product, $item['qty'], 'min', $discount ), wc_get_price_to_display( $_product, array(
			...
			if ( ! empty( $ids ) ) {
				...
				if ( is_array( $items ) && count( $items ) > 0 ) {
					foreach ( $items as $item ) {
						$_arr   = explode( '/', $item );
						$_id    = absint( isset( $_arr[0] ) ? $_arr[0] : 0 );
						$_variation_id = isset($_POST['variation_id_' . $_id]) && !empty($_POST['variation_id_' . $_id]) ? absint($_POST['variation_id_' . $_id]) : '';
						$_variation_attribute = isset($_POST['variation_attribute_' . $_id]) && !empty($_POST['variation_attribute_' . $_id]) ? $_POST['variation_attribute_' . $_id] : '';
						...
						if( $_variation_id ){
							try {
								$array['variation_id'] = $_variation_id;
								$adding_to_cart     = wc_get_product( $_id );
								$variations = array();
								...
								if( !empty($adding_to_cart) ){
									...
									foreach ( $adding_to_cart->get_attributes() as $attribute ) {
										...
										if ( isset( $_REQUEST[ 'variation_attribute_' . $_id ] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
											if ( $attribute['is_taxonomy'] ) {
												// Don't use wc_clean as it destroys sanitized characters.
												$value = sanitize_title( wp_unslash( $_REQUEST[ 'variation_attribute_' . $_id ] ) ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
											} else {
												$value = html_entity_decode( wc_clean( wp_unslash( $_REQUEST[ 'variation_attribute_' . $_id ] ) ), ENT_QUOTES, get_bloginfo( 'charset' ) ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
											}
								...
								if ( empty( $_variation_id ) ) {
									...
									$_variation_id = $data_store->find_matching_product_variation( $adding_to_cart, $posted_attributes );
								}
								...
								$variation_data = wc_get_product_variation_attributes( $_variation_id );
								...
								$array['variation_attribute'] = $variations;
