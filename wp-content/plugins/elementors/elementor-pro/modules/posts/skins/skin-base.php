...
abstract class Skin_Base extends Elementor_Skin_Base {
	...
	protected function register_meta_data_controls() {
        ...
        if ( class_exists( '\acf' ) && function_exists( 'acf_get_field_groups' ) ) {
            ...
            if( $get_queried_object instanceof \WP_Post_Type){
                ...
                $settings = $this->parent->get_settings();
                if (isset($settings['posts_post_type'])) {
            ...
            if (!empty($post_type)) {
                ...
                if (!empty($groups)) {
                    ...
                    foreach ($groups as $group){
                        $field_for_group = acf_get_fields($group['key']);
                        $fields = array_merge($fields, $field_for_group);
                    }
                    if( !empty($fields) ) {
	...
	protected function register_design_content_controls() {
        ...
        $this->add_control(
            'heading_meta_acf_style',
            [
                'label' => __( 'ACF', 'elementor-pro' ),
                'type' => Controls_Manager::HEADING,
                'separator' => 'before',
                'condition' => [
                    $this->get_control_id( 'meta_acf!' ) => [],
                ],
            ]
        );
        ...
        $this->add_control(
            ...
            [
                ...
                'type' => Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} .elementor-post__meta-acf-data' => 'color: {{VALUE}};',
                ],
                'condition' => [
                    $this->get_control_id( 'meta_acf!' ) => [],
                ],
        ...
        $this->add_group_control(
            Group_Control_Typography::get_type(),
            [
                'name' => 'meta_acf_typography',
                'scheme' => Schemes\Typography::TYPOGRAPHY_2,
                'selector' => '{{WRAPPER}} .elementor-post__meta-acf-data',
                'condition' => [
                    $this->get_control_id( 'meta_acf!' ) => [],
                ],
            ]
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .elementor-post__meta-acf-data' => 'margin-bottom: {{SIZE}}{{UNIT}};',
                ],
                'condition' => [
                    $this->get_control_id( 'meta_acf!' ) => [],
                ],
     ...
     * @TODO
     */
	protected function render_meta_acf() {

        if ( class_exists( '\acf' ) && function_exists( 'get_field_object' ) ) {
            $settings = $this->get_instance_value('meta_acf');
            if (empty($settings)) {
                return;
            }
            $post_ID = get_the_ID();
            ?>
            <div class="elementor-post__meta-acf-data">
                <?php
                foreach ($settings as $key) {
                    ...
                    if( !empty($field) ){
                        ...
                        switch ($field['type']){
                            ...
                            case 'page_link':
                                if( is_array($value) ){
                                    $value = '<a href="' . $value['url'] . '" target="' . $value['target'] . '">' . $value['title'] . '</a>';
                                }else{
                        ...
                        echo '<div class="field-item field-' . str_replace('_', '-', $field['name']) . '">' . $value . '</div>';
                    }
                }
                ?>
            </div>
	...
	protected function render_post() {
         ...
         * @TODO
         */
		$this->render_meta_acf();
		do_action('elementor/modules/posts', $this);
        /**
         * END @TODO
         */
		$this->render_excerpt();
