...
class Activate extends Common_App {
	...
	public function action_activate_license() {
		if ( ! $this->is_connected() ) {
			$this->add_notice( __( 'Please connect to Drag & Drop Layout in order to activate license.', 'elementor-pro' ), 'error' );
		...
		if ( empty( $license ) ) {
			...
			wp_die( 'License not found for user ' . $this->get( 'user' )->email, __( 'Drag & Drop Layout Pro', 'elementor-pro' ), [
				'back_link' => true,
			] );
		}

		if ( is_wp_error( $license ) ) {
			wp_die( $license, __( 'Drag & Drop Layout Pro', 'elementor-pro' ), [
				'back_link' => true,
			] );
		}
		...
		if ( empty( $license_key ) ) {
			wp_die( __( 'License key is missing.', 'elementor-pro' ), __( 'Drag & Drop Layout Pro', 'elementor-pro' ), [
				'back_link' => true,
			] );
		}
