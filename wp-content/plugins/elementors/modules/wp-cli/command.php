...
class Command extends \WP_CLI_Command {
	...
	/**
	 * Flush the Drag & Drop Layout Page Builder CSS Cache.
	 *
	 * [--network]
	...
	public function flush_css( $args, $assoc_args ) {
		...
		if ( $network ) {
			...
			foreach ( $blogs as $keys => $blog ) {
				...
				\WP_CLI::success( 'Flushed the Drag & Drop Layout CSS Cache for site - ' . get_option( 'home' ) );
			...
			\WP_CLI::success( 'Flushed the Drag & Drop Layout CSS Cache' );
		}
	}

	/**
	 * Replace old URLs with new URLs in all Drag & Drop Layout pages.
	 *
	 * ## EXAMPLES
	 *
