...
/**
 * Drag & Drop Layout history revisions manager.
 *
 * Drag & Drop Layout history revisions manager handler class is responsible for
 * registering and managing Drag & Drop Layout revisions manager.
 *
 * @since 1.7.0
 */
class Revisions_Manager {
	 ...
	 *
	 * Initializing Drag & Drop Layout history revisions manager.
	 *
	 * @since 1.7.0
	 * @access public
	 */
	...
	public static function remove_temp_post_content() {
		...
		if ( Plugin::$instance->db->is_built_with_elementor( $post->ID ) ) {
			$post->post_content = str_replace( '<!-- Created with Drag & Drop Layout -->', '', $post->post_content );
		}
	}
