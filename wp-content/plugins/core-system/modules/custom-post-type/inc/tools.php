 ...
 * @return void
 */
function cpt_tools_assets() {
	$current_screen = get_current_screen();

	if ( ! is_object( $current_screen ) || 'cpt_page_cpt_tools' !== $current_screen->base ) {
		return;
	}
...
add_action( 'admin_enqueue_scripts', 'cpt_tools_assets' );
 ...
 */
function cpt_tools_tabs( $tabs = [], $current_page = '' ) {
	...
	if ( 'tools' === $current_page ) {
		...
		$tabs['tabs']['post_types'] = [
			'text'          => __( 'Post Types', CPT_LANG ),
			'classes'       => $classes,
			'url'           => cpt_admin_url( 'admin.php?page=cpt_' . $current_page ),
			'aria-selected' => 'false',
		];

		$tabs['tabs']['taxonomies'] = [
			'text'          => __( 'Taxonomies', CPT_LANG ),
			'classes'       => $classes,
			'url'           => esc_url( add_query_arg( [ 'action' => 'taxonomies' ], cpt_admin_url( 'admin.php?page=cpt_' . $current_page ) ) ),
			'aria-selected' => 'false',
		];

		$tabs['tabs']['get_code'] = [
			'text'          => __( 'Get Code', CPT_LANG ),
			'classes'       => $classes,
			'url'           => esc_url( add_query_arg( [ 'action' => 'get_code' ], cpt_admin_url( 'admin.php?page=cpt_' . $current_page ) ) ),
			'aria-selected' => 'false',
		];

		/*$tabs['tabs']['debuginfo'] = [
			'text'          => __( 'Debug Info', CPT_LANG ),
			'classes'       => $classes,
			'url'           => esc_url( add_query_arg( [ 'action' => 'debuginfo' ], cpt_admin_url( 'admin.php?page=cpt_' . $current_page ) ) ),
			'aria-selected' => 'false',
		];*/
		 ...
		 */
		$tabs = apply_filters( 'cpt_tools_tabs', $tabs, $action, $active_class );
	}

	return $tabs;
}
add_filter( 'cpt_get_tabs', 'cpt_tools_tabs', 10, 2 );
...
function cpt_tools() {
	 ...
	 * @deprecated 1.5.0
	 */
	do_action_deprecated( 'cpt_inside_importexport_wrap', [], '1.5.0', 'cpt_inside_tools_wrap' );
	 ...
	 * @since 1.5.0
	 */
	do_action( 'cpt_inside_tools_wrap' );

	// Create our tabs.
	cpt_settings_tab_menu( 'tools' );
	 ...
	 */
	do_action_deprecated( 'cpt_import_export_sections', [ $tab ], '1.5.0', 'cpt_tools_sections' );
	 ...
	 * @param string $tab Current tab being displayed.
	 */
	do_action( 'cpt_tools_sections', $tab );
 ...
 *
 * @since 1.0.0
 * @since 1.2.0 Added $cpt_taxonomies parameter.
 * @since 1.2.0 Added $single parameter.
 *
 * @param array $cpt_taxonomies Array of taxonomies to render. Optional.
 * @param bool  $single           Whether or not we are rendering a single taxonomy. Optional. Default false.
 */
function cpt_get_taxonomy_code( $cpt_taxonomies = [], $single = false ) {
	if ( ! empty( $cpt_taxonomies ) ) {
		$callback = 'cpt_register_my_taxes';
		if ( $single ) {
			$key      = key( $cpt_taxonomies );
			$callback = 'cpt_register_my_taxes_' . str_replace( '-', '_', esc_html( $cpt_taxonomies[ $key ]['name'] ) );
		}
	?>
function <?php echo esc_html( $callback ); ?>() {
<?php
foreach ( $cpt_taxonomies as $tax ) {
	echo cpt_get_single_taxonomy_registery( $tax );
} ?>
}
add_action( 'init', '<?php echo esc_html( $callback ); ?>' );
<?php
	} else {
		esc_html_e( 'No taxonomies to display at this time', CPT_LANG );
	}
}
 ...
 */
function cpt_get_single_taxonomy_registery( $taxonomy = [] ) {
 ...
 *
 * @since 1.0.0
 * @since 1.2.0 Added $cpt_post_types parameter.
 * @since 1.2.0 Added $single parameter.
 *
 * @param array $cpt_post_types Array of post types to render. Optional.
 * @param bool  $single           Whether or not we are rendering a single post type. Optional. Default false.
 */
function cpt_get_post_type_code( $cpt_post_types = [], $single = false ) {
	// Whitespace very much matters here, thus why it's all flush against the left side.
	if ( ! empty( $cpt_post_types ) ) {
		$callback = 'cpt_register_my_cpts';
		if ( $single ) {
			$key      = key( $cpt_post_types );
			$callback = 'cpt_register_my_cpts_' . str_replace( '-', '_', esc_html( $cpt_post_types[ $key ]['name'] ) );
		}
?>
...
// Space before this line reflects in textarea.
		foreach ( $cpt_post_types as $type ) {
			echo cpt_get_single_post_type_registery( $type );
		}
?>
}
...
<?php
		...
		esc_html_e( 'No post types to display at this time', CPT_LANG );
	}
}
 ...
 */
function cpt_get_single_post_type_registery( $post_type = [] ) {
 ...
 */
function cpt_import_types_taxes_settings( $postdata = [] ) {
	if ( ! isset( $postdata['cpt_post_import'] ) && ! isset( $postdata['cpt_tax_import'] ) ) {
		return false;
	}

	if ( ! check_admin_referer( 'cpt_typetaximport_nonce_action', 'cpt_typetaximport_nonce_field' ) ) {
		return 'nonce_fail';
	}
	 ...
	 */
	$third_party_post_type_data = apply_filters( 'cpt_third_party_post_type_import', false );
	 ...
	 * @param bool $value Default to no data.
	 */
	$third_party_taxonomy_data  = apply_filters( 'cpt_third_party_taxonomy_import', false );

	if ( false !== $third_party_post_type_data ) {
		$postdata['cpt_post_import'] = $third_party_post_type_data;
	}

	if ( false !== $third_party_taxonomy_data ) {
		$postdata['cpt_tax_import'] = $third_party_taxonomy_data;
	}

	if ( ! empty( $postdata['cpt_post_import'] ) ) {
		$cpt_data = stripslashes_deep( trim( $postdata['cpt_post_import'] ) );
		...
		if ( null === $settings && '{""}' === $cpt_data ) {
			 ...
			 */
			if ( false === ( $success = apply_filters( 'cpt_post_type_import_delete_save', false, $postdata ) ) ) {
				$success = delete_option( 'cpt_post_types' );
			}
		}

		if ( $settings ) {
			if ( false !== cpt_get_post_type_data() ) {
				/** This filter is documented in /inc/import-export.php */
				if ( false === ( $success = apply_filters( 'cpt_post_type_import_delete_save', false, $postdata ) ) ) {
					delete_option( 'cpt_post_types' );
				}
			}
			 ...
			 * @param array $postdata Post type data.
			 */
			if ( false === ( $success = apply_filters( 'cpt_post_type_import_update_save', false, $postdata ) ) ) {
				$success = update_option( 'cpt_post_types', $settings );
			}
		}
	...
	} elseif ( ! empty( $postdata['cpt_tax_import'] ) ) {
		$tax_data = stripslashes_deep( trim( $postdata['cpt_tax_import'] ) );
		...
		if ( null === $settings && '{""}' === $tax_data ) {
			 ...
			 */
			if ( false === ( $success = apply_filters( 'cpt_taxonomy_import_delete_save', false, $postdata ) ) ) {
				$success = delete_option( 'cpt_taxonomies' );
			}
		}

		if ( $settings ) {
			if ( false !== cpt_get_taxonomy_data() ) {
				/** This filter is documented in /inc/import-export.php */
				if ( false === ( $success = apply_filters( 'cpt_taxonomy_import_delete_save', false, $postdata ) ) ) {
					delete_option( 'cpt_taxonomies' );
				}
			}
			/**
			 ...
			 * @param array $postdata Taxonomy data.
			 */
			if ( false === ( $success = apply_filters( 'cpt_taxonomy_import_update_save', false, $postdata ) ) ) {
				$success = update_option( 'cpt_taxonomies', $settings );
			}
		}
		// Used to help flush rewrite rules on init.
		set_transient( 'cpt_flush_rewrite_rules', 'true', 5 * 60 );
 ...
 * @internal
 */
function cpt_render_posttypes_taxonomies_section() {
?>

	<p><?php esc_html_e( 'If you are wanting to migrate registered post types or taxonomies from this site to another, that will also use Custom Post Type, use the import and export functionality. If you are moving away from Custom Post Type, use the information in the "Get Code" tab.', CPT_LANG ); ?></p>

<p>
<?php
	printf(
		'<strong>%s</strong>: %s',
		esc_html__( 'NOTE', CPT_LANG ),
		esc_html__( 'This will not export the associated posts or taxonomy terms, just the settings.', CPT_LANG )
	);
?>
</p>
<table class="form-table cpt-table">
	<?php if ( ! empty( $_GET ) && empty( $_GET['action'] ) ) { ?>
		<tr>
			<td class="outer">
				<h2><label for="cpt_post_import"><?php esc_html_e( 'Import Post Types', CPT_LANG ); ?></label></h2>

				<form method="post">
					<textarea class="cpt_post_import" placeholder="<?php esc_attr_e( 'Paste content here.', CPT_LANG ); ?>" id="cpt_post_import" name="cpt_post_import"></textarea>

					<p class="wp-ui-highlight">
						<strong><?php esc_html_e( 'Note:', CPT_LANG ); ?></strong> <?php esc_html_e( 'Importing will overwrite previous registered settings.', CPT_LANG ); ?>
					</p>

					<p>
						<strong><?php esc_html_e( 'To import post types from a different WordPress site, paste the exported content from that site and click the "Import" button.', CPT_LANG ); ?></strong>
					</p>

					<p>
						<input class="button button-primary" type="submit" value="<?php esc_attr_e( 'Import', CPT_LANG ); ?>" />
					</p>
					<?php wp_nonce_field( 'cpt_typetaximport_nonce_action', 'cpt_typetaximport_nonce_field' ); ?>
				</form>
			</td>
			<td class="outer">
				<h2><label for="cpt_post_export"><?php esc_html_e( 'Export Post Types settings', CPT_LANG ); ?></label></h2>
				<?php
				$cpt_post_types = cpt_get_post_type_data();
				if ( ! empty( $cpt_post_types ) ) {
					$content = esc_html( json_encode( $cpt_post_types ) );
				} else {
					$content = esc_html__( 'No post types registered yet.', CPT_LANG );
				}
				?>
				<textarea title="<?php esc_attr_e( 'To copy the system info, click below then press Ctrl + C (PC) or Cmd + C (Mac).', CPT_LANG ); ?>" onclick="this.focus();this.select();" onfocus="this.focus();this.select();" readonly="readonly" aria-readonly="true" class="cpt_post_import" id="cpt_post_export" name="cpt_post_export"><?php echo $content; // WPCS: XSS ok, sanitization ok. ?></textarea>

				<p>
					<strong><?php esc_html_e( 'Use the content above to import current post types into a different WordPress site. You can also use this to simply back up your post type settings.', CPT_LANG ); ?></strong>
				</p>
			</td>
		</tr>
	<?php } elseif ( ! empty( $_GET ) && 'taxonomies' === $_GET['action'] ) { ?>
		<tr>
			<td class="outer">
				<h2><label for="cpt_tax_import"><?php esc_html_e( 'Import Taxonomies', CPT_LANG ); ?></label></h2>

				<form method="post">
					<textarea class="cpt_tax_import" placeholder="<?php esc_attr_e( 'Paste content here.', CPT_LANG ); ?>" id="cpt_tax_import" name="cpt_tax_import"></textarea>
					...
					<p>
						<strong><?php esc_html_e( 'To import taxonomies from a different WordPress site, paste the exported content from that site and click the "Import" button.', CPT_LANG ); ?></strong>
					</p>
			...
			<td class="outer">
				<h2><label for="cpt_tax_export"><?php esc_html_e( 'Export Taxonomies settings', CPT_LANG ); ?></label></h2>
				<?php
				$cpt_taxonomies = cpt_get_taxonomy_data();
				if ( ! empty( $cpt_taxonomies ) ) {
					$content = esc_html( json_encode( $cpt_taxonomies ) );
				} else {
					$content = esc_html__( 'No taxonomies registered yet.', CPT_LANG );
				}
				?>
				<textarea title="<?php esc_attr_e( 'To copy the system info, click below then press Ctrl + C (PC) or Cmd + C (Mac).', CPT_LANG ); ?>" onclick="this.focus();this.select()" onfocus="this.focus();this.select();" readonly="readonly" aria-readonly="true" class="cpt_tax_import" id="cpt_tax_export" name="cpt_tax_export"><?php echo $content; //WPCS: XSS ok, sanitization ok.?></textarea>

				<p>
					<strong><?php esc_html_e( 'Use the content above to import current taxonomies into a different WordPress site. You can also use this to simply back up your taxonomy settings.', CPT_LANG ); ?></strong>
				</p>
			</td>
		</tr>
	<?php } ?>
 ...
 * @internal
 */
function cpt_render_getcode_section() {
?>
	<h1><?php esc_html_e( 'Get Post Type and Taxonomy Code', CPT_LANG ); ?></h1>

		<h2><?php esc_html_e( 'All Custom Post Type Post Types', CPT_LANG ); ?></h2>

		<p><?php esc_html_e( 'All of the selectable code snippets below are useful if you wish to migrate away from Custom Post Type and retain your existing registered post types or taxonomies.', CPT_LANG ); ?></p>

		<?php $cpt_post_types = cpt_get_post_type_data(); ?>
		<p><label for="cpt_post_type_get_code"><?php esc_html_e( 'Copy/paste the code below into your functions.php file.', CPT_LANG ); ?></label></p>
		<textarea name="cpt_post_type_get_code" id="cpt_post_type_get_code" class="large-text cpt_post_type_get_code" onclick="this.focus();this.select()" onfocus="this.focus();this.select();" readonly="readonly" aria-readonly="true"><?php cpt_get_post_type_code( $cpt_post_types ); ?></textarea>
		...
		if ( ! empty( $cpt_post_types ) ) {
			...
			?>
				<h2 id="<?php echo esc_attr( $post_type['name'] ); ?>">
					...
					printf( esc_html__( '%s Post Type', CPT_LANG ), esc_html( $type ) ); ?></h2>
				<p><label for="cpt_post_type_get_code_<?php echo esc_attr( $post_type['name'] ); ?>"><?php esc_html_e( 'Copy/paste the code below into your functions.php file.', CPT_LANG ); ?></label></p>
				<textarea name="cpt_post_type_get_code_<?php echo esc_attr( $post_type['name'] ); ?>" id="cpt_post_type_get_code_<?php echo esc_attr( $post_type['name'] ); ?>" class="large-text cpt_post_type_get_code" onclick="this.focus();this.select()" onfocus="this.focus();this.select();" readonly="readonly" aria-readonly="true"><?php cpt_get_post_type_code( [ $post_type ], true ); ?></textarea>
			<?php
			}
		}
		?>

		<h2><?php esc_html_e( 'All Custom Post Type Taxonomies', CPT_LANG ); ?></h2>

		<?php $cpt_taxonomies = cpt_get_taxonomy_data(); ?>
		<p><label for="cpt_tax_get_code"><?php esc_html_e( 'Copy/paste the code below into your functions.php file.', CPT_LANG ); ?></label></p>
		<textarea name="cpt_tax_get_code" id="cpt_tax_get_code" class="large-text cpt_tax_get_code" onclick="this.focus();this.select()" onfocus="this.focus();this.select();" readonly="readonly" aria-readonly="true"><?php cpt_get_taxonomy_code( $cpt_taxonomies ); ?></textarea>
		...
		if ( ! empty( $cpt_taxonomies ) ) {
			...
			?>
				<h2 id="<?php echo esc_attr( $taxonomy['name'] ); ?>">
					...
					printf( esc_html__( '%s Taxonomy', CPT_LANG ), esc_html( $tax ) );
					?>
				</h2>
				<p><label for="cpt_tax_get_code_<?php echo esc_attr( $taxonomy['name'] ); ?>"><?php esc_html_e( 'Copy/paste the code below into your functions.php file.', CPT_LANG ); ?></label></p>
				<textarea name="cpt_tax_get_code_<?php echo esc_attr( $taxonomy['name'] ); ?>" id="cpt_tax_get_code_<?php echo esc_attr( $taxonomy['name'] ); ?>" class="large-text cpt_tax_get_code" onclick="this.focus();this.select()" onfocus="this.focus();this.select();" readonly="readonly" aria-readonly="true"><?php cpt_get_taxonomy_code( [ $taxonomy ], true ); ?></textarea>
			<?php
			}
		}
		?>
 ...
 * @internal
 */
function cpt_render_debuginfo_section() {

	$debuginfo = new \CoreSystem\Modules\CustomPostType\Classes\CPT_Debug_Info();

	echo '<form id="cpt_debug_info" method="post">';
	$debuginfo->tab_site_info();

	wp_nonce_field( 'cpt_debuginfo_nonce_action', 'cpt_debuginfo_nonce_field' );

	if ( ! empty( $_POST ) && isset( $_POST['cpt_debug_info_email'] ) && isset( $_POST['cpt_debuginfo_nonce_field'] ) ) {
		wp_verify_nonce( 'cpt_debuginfo_nonce_field', 'cpt_debuginfo_nonce_action' );

		$email_args          = [];
		$email_args['email'] = sanitize_text_field( $_POST['cpt_debug_info_email'] );
		$debuginfo->send_email( $email_args );
	}

	echo '<p><label for="cpt_debug_info_email">' . esc_html__( 'Please provide an email address to send debug information to: ', CPT_LANG ) . '</label><input type="email" id="cpt_debug_info_email" name="cpt_debug_info_email" value="" /></p>';
	 ...
	 */
	echo '<p><input type="submit" class="button-primary" name="cpt_send_debug_email" value="' . esc_attr( apply_filters( 'cpt_debug_email_submit_button', __( 'Send debug info', CPT_LANG ) ) ) . '" /></p>';
	echo '</form>';
 ...
 */
function cpt_render_tools( $tab ) {
	if ( isset( $tab ) ) {
		if ( 'post_types' === $tab || 'taxonomies' === $tab ) {
			cpt_render_posttypes_taxonomies_section();
		}

		if ( 'get_code' === $tab ) {
			cpt_render_getcode_section();
		}

		if ( 'debuginfo' === $tab ) {
			cpt_render_debuginfo_section();
		}
	}
}
add_action( 'cpt_tools_sections', 'cpt_render_tools' );
 ...
 * @since 1.5.0
 */
function cpt_do_import_types_taxes() {

	if ( ! empty( $_POST ) &&
	     ( ! empty( $_POST['cpt_post_import'] ) && isset( $_POST['cpt_post_import'] ) ) ||
	     ( ! empty( $_POST['cpt_tax_import'] ) && isset( $_POST['cpt_tax_import'] ) )
	) {
		$success = cpt_import_types_taxes_settings( $_POST );
		add_action( 'admin_notices', "cpt_{$success}_admin_notice" );
	}
}
add_action( 'init', 'cpt_do_import_types_taxes', 8 );
