<?php
/**
 * Custom Post Type Debug Information.
 *
 * @package CPT
...
namespace CoreSystem\Modules\CustomPostType\Classes;
/**
 * Custom Post Type Debug Info
 */
class CPT_Debug_Info {
	...
	public function tab_site_info() {
		?>
		<p><?php _e( 'If you have sought support for Custom Post Type on the forums, you may be requested to send the information below to the plugin developer. Simply insert the email they provided in the input field at the bottom and click the "Send debug info" button. Only the data below will be sent to them.', CPT_LANG ); ?></p>
		<label for="cpt_audit_textarea">
		<textarea readonly="readonly" aria-readonly="true" id="cpt-audit-textarea" name="cpt_audit_textarea" rows="20" cols="100" class="large-text code">
	...
	private function system_status() {
		...
		### Begin Custom Post Type Debug Info ###
		...
		echo "\t\t" . 'Post Types: ' . "\n";
		echo "\t\t" . json_encode( $cpt_post_types ) . "\n";

		echo "\n\n";

		$cpt_taxonomies = cpt_get_taxonomy_data();
		echo "\t\t" . 'Taxonomies: ' . "\n";
		echo "\t\t" . json_encode( $cpt_taxonomies ) . "\n";
		echo "\n";

		if ( has_action( 'cpt_custom_debug_info' ) ) {
		 ...
		 * @since 1.3.0
		 */
		do_action( 'cpt_custom_debug_info' );
	...
	public function send_email( $args = [] ) {
		 ...
		 */
		$subject = apply_filters( 'cpt_debug_email_subject', sprintf(
			// translators: Placeholder will hold site home_url.
			__( 'Custom Post Type debug information for %s', CPT_LANG ),
			home_url( '/' )
		) );
		 ...
		 * @since 1.3.0
		 */
		do_action( 'cpt_after_debug_email_sent' );
