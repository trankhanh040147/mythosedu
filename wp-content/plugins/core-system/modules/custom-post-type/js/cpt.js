...
(function($) {
	...
	$('#cpt_select_post_type_submit').hide();
	$('#cpt_select_taxonomy_submit').hide();
	...
	$('#post_type').on('change',function(){
		$('#cpt_select_post_type').submit();
	});

	$('#taxonomy').on('change',function(){
		$( '#cpt_select_taxonomy' ).submit();
	});

	// Confirm our deletions
	$('.cpt-delete-top, .cpt-delete-bottom').on('click',function(e) {
		e.preventDefault();
		var msg = '';
		if (typeof cpt_type_data !== 'undefined') {
			msg = cpt_type_data.confirm;
		} else if (typeof cpt_tax_data !== 'undefined') {
			msg = cpt_tax_data.confirm;
		}
		var submit_delete_warning = $('<div class="cpt-submit-delete-dialog">' + msg + '</div>').appendTo('#poststuff').dialog({
	...
	$('#name').on('keyup',function(e){
		...
		if ( typeof cpt_type_data != 'undefined' ) {
			if (cpt_type_data.existing_post_types.hasOwnProperty(value) && value !== original_slug) {
				$slugexists.removeClass('hidemessage');
			} else {
				$slugexists.addClass('hidemessage');
			}
		}
		if ( typeof cpt_tax_data != 'undefined' ) {
			if (cpt_tax_data.existing_taxonomies.hasOwnProperty(value) && value !== original_slug) {
				$slugexists.removeClass('hidemessage');
			} else {
				$slugexists.addClass('hidemessage');
			}
	...
	$('#cpt_choose_icon').on('click',function(e){
		e.preventDefault();
	...
	$('.cpt-help').on('click',function(e){
		e.preventDefault();
	});

	$('.cpt-taxonomy-submit').on('click',function(e){
		if ( $('.cpt-table :checkbox:checked').length == 0 ) {
			e.preventDefault();
			var no_associated_type_warning = $('<div class="cpt-taxonomy-empty-types-dialog">' + cpt_tax_data.no_associated_type + '</div>').appendTo('#poststuff').dialog({
				'dialogClass'   : 'wp-dialog',
				'modal'         : true,
				'autoOpen'      : true,
				'buttons'       : {
	...
	$('#auto-populate').on( 'click tap', function(e){
		...
		var fields   = $('.cpt-labels input[type="text"]');
