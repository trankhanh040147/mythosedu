...
function (_elementorModules$Mod) {
  ...
  (0, _createClass2.default)(ComponentBase, [{
    ...
    value: function activateTab(tab) {
      ...
      this.currentTab = tab;
      this.renderTab(tab);
      jQuery(this.getTabsWrapperSelector() + ' .co-component-tab').off('click').on('click', function (event) {
        $e.route(_this4.getTabRoute(event.currentTarget.dataset.tab));
      }).removeClass('co-active').filter('[data-tab="' + tab + '"]').addClass('co-active');
    }
  }, {
...
var SectionView = BaseElementView.extend({
  childViewContainer: '> .co-container > .co-row',
  template: Marionette.TemplateCache.get('#tmpl-co-section-content'),
  addSectionView: null,
  ...
  toggleSectionIsFull: function toggleSectionIsFull() {
    this.$el.toggleClass('co-section-filled', this.isCollectionFilled());
  },
  ...
  className: function className() {
    ...
    return classes + ' co-section co-' + type + '-section';
  },
  ...
  getSortableOptions: function getSortableOptions() {
    var sectionConnectClass = this.isInner() ? '.co-inner-section' : '.co-top-section';
    return {
      connectWith: sectionConnectClass + ' > .co-container > .co-row',
      handle: '> .co-element-overlay .co-editor-element-edit',
      items: '> .co-column',
...
function (_Marionette$ItemView) {
  ...
  (0, _createClass2.default)(AddSectionBase, [{
    ...
    value: function template() {
      return Marionette.TemplateCache.get('#tmpl-co-add-section');
    }
  }, {
    key: "attributes",
    ...
    value: function ui() {
      return {
        addNewSection: '.co-add-new-section',
        closeButton: '.co-add-section-close',
        addSectionButton: '.co-add-section-button',
        addTemplateButton: '.co-add-template-button',
        selectPreset: '.co-select-preset',
        presets: '.co-preset'
      };
    }
  }, {
    ...
    value: function className() {
      return 'co-add-section co-visible-desktop';
    }
  }, {
    key: "setView",
    ...
    value: function onRender() {
      this.$el.html5Droppable({
        ...
        groups: ['co-element'],
        placeholder: false,
        currentElementClass: 'co-html5dnd-current-element',
        hasDraggingOnChildClass: 'co-dragging-on-child',
...
BaseElementView = BaseContainer.extend({
  ...
  renderAttributes: {},
  className: function className() {
    var classes = 'co-element co-element-edit-mode ' + this.getElementUniqueID();

    if (this.toggleEditTools) {
      classes += ' co-element--toggle-edit-tools';
    }
  ...
  ui: function ui() {
    return {
      tools: '> .co-element-overlay > .co-editor-element-settings',
      editButton: '> .co-element-overlay .co-editor-element-edit',
      duplicateButton: '> .co-element-overlay .co-editor-element-duplicate',
      addButton: '> .co-element-overlay .co-editor-element-add',
      removeButton: '> .co-element-overlay .co-editor-element-remove'
    };
  },
  behaviors: function behaviors() {
  ...
  getHandlesOverlay: function getHandlesOverlay() {
    var $handlesOverlay = jQuery('<div>', {
      class: 'co-element-overlay'
    }),
        $overlayList = jQuery('<ul>', {
      class: "co-editor-element-settings co-editor-".concat(this.getElementType(), "-settings")
    });
    jQuery.each(this.getEditButtons(), function (toolName, tool) {
      var $item = jQuery('<li>', {
        class: "co-editor-element-setting co-editor-element-".concat(toolName),
        title: tool.title
      }),
          $icon = jQuery('<i>', {
  ...
  toggleVisibilityClass: function toggleVisibilityClass() {
    this.$el.toggleClass('co-edit-hidden', !!this.model.get('hidden'));
  },
  ...
  renderStyles: function renderStyles(settings) {
    ...
    this.controlsCSSParser.addStyleRules(settings.getStyleControls(), settings.attributes, this.getEditModel().get('settings').controls, [/{{ID}}/g, /{{WRAPPER}}/g], [this.getID(), '.co-' + elementor.config.document.id + ' .co-element.' + this.getElementUniqueID()]);
  ...
  runReadyTrigger: function runReadyTrigger() {
    ...
    _.defer(function () {
      ...
      self.$el.find('.co-element.co-' + self.model.get('elType') + ':not(.co-element-edit-mode)').each(function () {
  ...
  getDynamicParsingSettings: function getDynamicParsingSettings() {
    ...
    return {
      onServerRequestStart: function onServerRequestStart() {
        self.$el.addClass('co-loading');
      },
      onServerRequestEnd: function onServerRequestEnd() {
        self.render();
        self.$el.removeClass('co-loading');
      }
    };
  },
    ...
    if (this.toggleEditTools) {
      ...
      if (this.ui.tools) {
        this.ui.tools.hoverIntent(function () {
          editButton.addClass('co-active');
        }, function () {
          editButton.removeClass('co-active');
        }, {
          timeout: 500
        });
      }
  ...
  onMouseDown: function onMouseDown(event) {
    if (jQuery(event.target).closest('.co-inline-editing').length) {
      return;
    }
...
SortableBehavior = Marionette.Behavior.extend({
  ...
  activate: function activate() {
    ...
    var $childViewContainer = this.getChildViewContainer(),
      ...
      placeholder: 'co-sortable-placeholder co-' + this.getOption('elChildType') + '-placeholder',
      cursorAt: {
        top: 20,
        left: 25
      },
      helper: this._getSortableHelper.bind(this),
      cancel: 'input, textarea, button, select, option, .co-inline-editing, .co-tab-title'
    },
  ...
  _getSortableHelper: function _getSortableHelper(event, $item) {
    ...
    return '<div style="height: 84px; width: 125px;" class="co-sortable-helper co-sortable-helper-' + model.get('elType') + '"><div class="icon"><i class="' + model.getIcon() + '"></i></div><div class="co-element-title-wrapper"><div class="title">' + model.getTitle() + '</div></div></div>';
  },
  ...
  onSortOver: function onSortOver(event) {
    event.stopPropagation();
    var model = elementor.channels.data.request('dragging:model');
    jQuery(event.target).addClass('co-draggable-over').attr({
      'data-dragged-element': model.get('elType'),
      'data-dragged-is-inner': model.get('isInner')
    });
    this.$el.addClass('co-dragging-on-child');
  },
  onSortOut: function onSortOut(event) {
    event.stopPropagation();
    jQuery(event.target).removeClass('co-draggable-over').removeAttr('data-dragged-element data-dragged-is-inner');
    this.$el.removeClass('co-dragging-on-child');
  },
...
ControlsCSSParser = elementorModules.ViewModule.extend({
  ...
  getDefaultElements: function getDefaultElements() {
    var id = "co-style-".concat(this.getSettings('id'));
  ...
  addRepeaterControlsStyleRules: function addRepeaterControlsStyleRules(repeaterValues, repeaterControlsItems, controls, placeholders, replacements) {
    ...
    repeaterControlsItems.forEach(function (item, index) {
      ...
      self.addStyleRules(item, itemModel.attributes, controls, placeholders.concat(['{{CURRENT_ITEM}}']), replacements.concat(['.co-repeater-item-' + itemModel.get('_id')]));
    });
  },
  addDynamicControlStyleRules: function addDynamicControlStyleRules(value, control) {
    ...
    elementor.dynamicTags.parseTagsText(value, control.dynamic, function (id, name, settings) {
      ...
      self.addStyleRules(tagSettingsModel.getStyleControls(), tagSettingsModel.attributes, tagSettingsModel.controls, ['{{WRAPPER}}'], ['#co-tag-' + id]);
    });
  },
...
module.exports = elementorModules.Module.extend({
  getDefaultSettings: function getDefaultSettings() {
    return {
      actions: {},
      classes: {
        list: 'co-context-menu-list',
        group: 'co-context-menu-list__group',
        groupPrefix: 'co-context-menu-list__group-',
        item: 'co-context-menu-list__item',
        itemTypePrefix: 'co-context-menu-list__item-',
        itemTitle: 'co-context-menu-list__item__title',
        itemShortcut: 'co-context-menu-list__item__shortcut',
        iconShortcut: 'co-context-menu-list__item__icon',
        itemDisabled: 'co-context-menu-list__item--disabled',
        divider: 'co-context-menu-list__divider',
        hidden: 'co-hidden'
      }
    };
  },
...
function (_BaseAddSectionView) {
  ...
  (0, _createClass2.default)(AddSectionView, [{
    key: "className",
    value: function className() {
      return (0, _get2.default)((0, _getPrototypeOf2.default)(AddSectionView.prototype), "className", this).call(this) + ' co-add-section-inline';
    }
  }, {
    key: "fadeToDeath",
...
ColumnView = BaseElementView.extend({
  template: Marionette.TemplateCache.get('#tmpl-co-column-content'),
  emptyView: ColumnEmptyView,
  childViewContainer: '> .co-column-wrap > .co-widget-wrap',
  ...
  className: function className() {
    var classes = BaseElementView.prototype.className.apply(this, arguments),
        type = this.isInner() ? 'inner' : 'top';
    return classes + ' co-column co-' + type + '-column';
  },
  tagName: function tagName() {
  ...
  ui: function ui() {
    ...
    ui.columnInner = '> .co-column-wrap';
    ui.percentsTooltip = '> .co-element-overlay .co-column-percents-tooltip';
    return ui;
  },
  getEditButtons: function getEditButtons() {
    var elementData = elementor.getElementData(this.model),
  ...
  attachElContent: function attachElContent() {
    ...
    var $tooltip = jQuery('<div>', {
      class: 'co-column-percents-tooltip'
    });
    this.$el.children('.co-element-overlay').append($tooltip);
  },
  getContextMenuGroups: function getContextMenuGroups() {
    var self = this,
  ...
  getSortableOptions: function getSortableOptions() {
    return {
      connectWith: '.co-widget-wrap',
      items: '> .co-element'
    };
  },
  changeChildContainerClasses: function changeChildContainerClasses() {
    var emptyClass = 'co-element-empty',
        populatedClass = 'co-element-populated';
  ...
  onRender: function onRender() {
    ...
    self.$el.html5Droppable({
      items: ' > .co-column-wrap > .co-widget-wrap > .co-element, >.co-column-wrap > .co-widget-wrap > .co-empty-view > .co-first-add',
      axis: ['vertical'],
      groups: ['co-element'],
      isDroppingAllowed: self.isDroppingAllowed.bind(self),
      currentElementClass: 'co-html5dnd-current-element',
      placeholderClass: 'co-sortable-placeholder co-widget-placeholder',
      hasDraggingOnChildClass: 'co-dragging-on-child',
...
module.exports = Marionette.ItemView.extend({
  template: '#tmpl-co-empty-preview',
  className: 'co-empty-view',
  events: {
    click: 'onClickAdd'
  },
  behaviors: function behaviors() {
  ...
  (0, _createClass2.default)(ResizeColumn, [{
    ...
    value: function resizeColumn(container, newSize) {
      ...
      if (undefined === container.oldValues || null === container.oldValues._inline_size) {
        ...
        var totalWidth = parentView.$el.find(' > .co-container')[0].getBoundingClientRect().width;
...
function (_HookUIAfter) {
  ...
  (0, _createClass2.default)(FooterSaverAfterSave, [{
    ...
    value: function apply(args, result) {
      ...
      NProgress.done();
      footerSaver.ui.buttonPublish.removeClass('co-button-state');
      footerSaver.ui.lastEditedWrapper.removeClass('co-state-active');
...
function (_HookUIBefore) {
  ...
  (0, _createClass2.default)(FooterSaverBeforeSave, [{
    ...
    value: function apply(args) {
      ...
      if ('autosave' === status) {
        footerSaver.ui.lastEditedWrapper.addClass('co-state-active');
      } else {
        footerSaver.ui.buttonPublish.addClass('co-button-state');
      }
    }
  }]);
...
function (_HookUICatch) {
  ...
  (0, _createClass2.default)(FooterSaverCatchSave, [{
    ...
    value: function apply() {
      NProgress.done();
      $e.components.get('document/save').footerSaver.ui.buttonPublish.removeClass('co-button-state');
    }
  }]);
