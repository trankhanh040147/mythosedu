...
class Module extends \Elementor\Core\Base\Module {
	...
	public function ajax_enable_safe_mode( $data ) {
		...
		if ( $document ) {
			return add_query_arg( 'co-mode', 'safe', $document->get_edit_url() );
		}
	...
	public function filter_preview_url( $url ) {
		return add_query_arg( 'co-mode', 'safe', $url );
	}
	...
	public function print_safe_mode_css() {
		...
		<style>
			...
			body.rtl .co-safe-mode-toast {
				left: 10px;
			}

			body:not(.rtl) .co-safe-mode-toast {
				right: 10px;
			}
			...
			.co-safe-mode-toast .co-toast-content {
				font-size: 13px;
				line-height: 22px;
				color: #6D7882;
			}

			.co-safe-mode-toast .co-toast-content a {
				color: #138FFF;
			}

			.co-safe-mode-toast .co-toast-content hr {
				margin: 15px auto;
				border: 0 none;
				border-top: 1px solid #F1F3F5;
			}

			.co-safe-mode-toast header {
				display: flex;
				align-items: center;
			...
			.co-safe-mode-toast .co-safe-mode-button {
			...
			#co-try-safe-mode .co-safe-mode-button {
				background: #39B54A;
			}

			.co-safe-mode-toast header i {
				font-size: 25px;
				color: #fcb92c;
			}

			body:not(.rtl) .co-safe-mode-toast header i {
				margin-right: 10px;
			}

			body.rtl .co-safe-mode-toast header i {
				margin-left: 10px;
			}

			.co-safe-mode-toast header h2 {
				flex-grow: 1;
				font-size: 18px;
				color: #6D7882;
			}

			.co-safe-mode-list-item {
				margin-top: 10px;
				list-style: outside;
			}

			body:not(.rtl) .co-safe-mode-list-item {
				margin-left: 15px;
			}

			body.rtl .co-safe-mode-list-item {
				margin-right: 15px;
			}

			.co-safe-mode-list-item b {
				font-size: 14px;
			}

			.co-safe-mode-list-item-content {
				font-style: italic;
				color: #a4afb7;
			}

			.co-safe-mode-list-item-title {
				font-weight: 500;
			}

			.co-safe-mode-mu-plugins {
	...
	public function print_safe_mode_notice() {
		...
		?>
		<div class="co-safe-mode-toast" id="co-safe-mode-message">
			<header>
				<i class="eicon-warning"></i>
				<h2><?php echo __( 'Safe Mode ON', 'elementor' ); ?></h2>
				<a class="co-safe-mode-button co-disable-safe-mode" target="_blank" href="<?php echo $this->get_admin_page_url(); ?>">
					<?php echo __( 'Disable Safe Mode', 'elementor' ); ?>
				</a>
			</header>

			<div class="co-toast-content">
				<ul class="co-safe-mode-list">
					<li class="co-safe-mode-list-item">
						<div class="co-safe-mode-list-item-title"><?php echo __( 'Editor successfully loaded?', 'elementor' ); ?></div>
						<div class="co-safe-mode-list-item-content"><?php echo __( 'The issue was probably caused by one of your plugins or theme.', 'elementor' ); ?> <?php printf( __( '<a href="%s" target="_blank">Click here</a> to troubleshoot', 'elementor' ), self::DOCS_HELPED_URL ); ?></div>
					</li>
					<li class="co-safe-mode-list-item">
						<div class="co-safe-mode-list-item-title"><?php echo __( 'Still experiencing issues?', 'elementor' ); ?></div>
						<div class="co-safe-mode-list-item-content"><?php printf( __( '<a href="%s" target="_blank">Click here</a> to troubleshoot', 'elementor' ), self::DOCS_DIDNT_HELP_URL ); ?></div>
					</li>
				</ul>
				<?php
				...
				if ( 1 < count( $mu_plugins ) ) : ?>
					<div class="co-safe-mode-mu-plugins"><?php printf( __( 'Please note! We couldn\'t deactivate all of your plugins on Safe Mode. Please <a href="%s" target="_blank">read more</a> about this issue.', 'elementor' ), self::DOCS_MU_PLUGINS_URL ); ?></div>
				<?php endif; ?>
			</div>
		</div>
		...
		<script>
			var ElementorSafeMode = function() {
				var attachEvents = function() {
				  jQuery( '.co-disable-safe-mode' ).on( 'click', function( e ) {
						...
						elementorCommon.ajax.addRequest(
							'disable_safe_mode', {
								success: function() {
									if ( -1 === location.href.indexOf( 'co-mode=safe' ) ) {
										location.reload();
									} else {
										// Need to remove the URL from browser history.
										location.replace( location.href.replace( '&co-mode=safe', '' ) );
									}
								},
								error: function() {
	...
	public function print_try_safe_mode() {
		...
		echo $this->print_safe_mode_css();
		?>
		<div class="co-safe-mode-toast" id="co-try-safe-mode">
		<?php if ( current_user_can( 'install_plugins' ) ) : ?>
			<header>
				<i class="eicon-warning"></i>
				<h2><?php echo __( 'Can\'t Edit?', 'elementor' ); ?></h2>
				<a class="co-safe-mode-button co-enable-safe-mode" target="_blank" href="<?php echo $this->get_admin_page_url(); ?>">
		...
		<script>
			var ElementorTrySafeMode = function() {
				var attachEvents = function() {
					jQuery( '.co-enable-safe-mode' ).on( 'click', function( e ) {
						if ( ! elementorCommon || ! elementorCommon.ajax ) {
							return;
						}
				...
				var isElementorLoaded = function() {
					...
					if ( jQuery( '#co-loading' ).is( ':visible' ) ) {
						return false;
					}
				...
				var handleTrySafeModeNotice = function() {
					var $notice = jQuery( '#co-try-safe-mode' );
	...
	public function register_scripts() {
		wp_add_inline_script( 'co-common', 'elementorCommon.ajax.addRequestConstant( "co-mode", "safe" );' );
	}
