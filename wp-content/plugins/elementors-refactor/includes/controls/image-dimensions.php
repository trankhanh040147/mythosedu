...
/**
 * Drag & Drop Layout image dimensions control.
 *
...
class Control_Image_Dimensions extends Control_Base_Multiple {
	...
	public function content_template() {
		if ( ! $this->is_image_editor_supports() ) : ?>
			<div class="co-panel-alert co-panel-alert-danger">
		...
		<div class="co-control-field">
			...
			<div class="co-control-input-wrapper">
				<div class="co-image-dimensions-field co-control-unit-2">
					<?php $control_uid = $this->get_control_uid( 'width' ); ?>
					<input id="<?php echo $control_uid; ?>" type="text" data-setting="width" />
					<label for="<?php echo $control_uid; ?>" class="co-image-dimensions-field-description"><?php echo __( 'Width', 'elementor' ); ?></label>
				</div>
				<div class="co-image-dimensions-separator">x</div>
				<div class="co-image-dimensions-field co-control-unit-2">
					<?php $control_uid = $this->get_control_uid( 'height' ); ?>
					<input id="<?php echo $control_uid; ?>" type="text" data-setting="height" />
					<label for="<?php echo $control_uid; ?>" class="co-image-dimensions-field-description"><?php echo __( 'Height', 'elementor' ); ?></label>
				</div>
				<button class="co-button co-button-success co-image-dimensions-apply-button"><?php echo __( 'Apply', 'elementor' ); ?></button>
			</div>
		</div>
		<?php
	}
