...
/**
 * Drag & Drop Layout number control.
 *
	...
	public function content_template() {
		...
		<div class="co-control-field">
			...
			<div class="co-control-input-wrapper co-control-dynamic-switcher-wrapper">
				<input id="<?php echo $control_uid; ?>" type="number" min="{{ data.min }}" max="{{ data.max }}" step="{{ data.step }}" class="tooltip-target co-control-tag-area co-control-unit-2" data-tooltip="{{ data.title }}" title="{{ data.title }}" data-setting="{{ data.name }}" placeholder="{{ data.placeholder }}" />
			</div>
		</div>
		<# if ( data.description ) { #>
