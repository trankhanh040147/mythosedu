...
class Control_Text extends Base_Data_Control {
	...
	public function content_template() {
		...
		<div class="co-control-field">
			...
			<# } #>
			<div class="co-control-input-wrapper co-control-unit-5 co-control-dynamic-switcher-wrapper">
				<input id="<?php echo $control_uid; ?>" type="{{ data.input_type }}" class="tooltip-target co-control-tag-area" data-tooltip="{{ data.title }}" title="{{ data.title }}" data-setting="{{ data.name }}" placeholder="{{ data.placeholder }}" />
			</div>
		</div>
		<# if ( data.description ) { #>
