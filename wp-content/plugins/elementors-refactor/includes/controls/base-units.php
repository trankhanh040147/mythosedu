...
/**
 * Drag & Drop Layout control base units.
 *
...
abstract class Control_Base_Units extends Control_Base_Multiple {
	...
	protected function print_units_template() {
		...
		<div class="co-units-choices">
			<# _.each( data.size_units, function( unit ) { #>
			<input id="co-choose-{{ data._cid + data.name + unit }}" type="radio" name="co-choose-{{ data.name }}" data-setting="unit" value="{{ unit }}">
			<label class="co-units-choices-label" for="co-choose-{{ data._cid + data.name + unit }}">{{{ unit }}}</label>
			<# } ); #>
		</div>
		<# } #>
		<?php
