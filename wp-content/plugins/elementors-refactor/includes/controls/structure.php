...
/**
 * Drag & Drop Layout structure control.
 *
		...
		<div class="co-control-field">
			<div class="co-control-input-wrapper">
				...
				if ( morePresets.length ) { #>
					<div class="co-control-structure-presets">
						<# _.each( morePresets, function( preset ) { #>
							<div class="co-control-structure-preset-wrapper">
								<input id="<?php echo $preset_control_uid; ?>" type="radio" name="co-control-structure-preset-{{ data._cid }}" data-setting="structure" value="{{ preset.key }}">
								<label for="<?php echo $preset_control_uid; ?>" class="co-control-structure-preset">
									{{{ elementor.presetsFactory.getPresetSVG( preset.preset, 102, 42 ).outerHTML }}}
								</label>
								<div class="co-control-structure-preset-title">{{{ preset.preset.join( ', ' ) }}}</div>
							</div>
						<# } ); #>
					</div>
				<# } #>
			</div>
			<div class="co-control-structure-reset">
