...
/**
 * Drag & Drop Layout slider control.
 *
...
class Control_Slider extends Control_Base_Units {
	...
	public function content_template() {
		...
		<div class="co-control-field">
			...
			<div class="co-control-input-wrapper co-control-dynamic-switcher-wrapper co-clearfix co-control-tag-area">
				<# if ( isMultiple && ( data.labels.length || data.scales ) ) { #>
					<div class="co-slider__extra">
						<# if ( data.labels.length ) { #>
						<div class="co-slider__labels">
							<# jQuery.each( data.labels, ( index, label ) => { #>
								<div class="co-slider__label">{{{ label }}}</div>
							<# } ); #>
						</div>
						<# } if ( data.scales ) { #>
						<div class="co-slider__scales">
							<# for ( var i = 0; i < data.scales; i++ ) { #>
								<div class="co-slider__scale"></div>
							<# } #>
						</div>
						<# } #>
					</div>
				<# } #>
				<div class="co-slider"></div>
				<# if ( ! isMultiple ) { #>
