...
class Control_URL extends Control_Base_Multiple {
	...
	public function content_template() {
		...
		?>
		<div class="co-control-field co-control-url-external-{{{ ( data.options.length || data.show_external ) ? 'show' : 'hide' }}}">
			<label for="<?php echo $control_uid; ?>" class="co-control-title">{{{ data.label }}}</label>
			<div class="co-control-input-wrapper co-control-dynamic-switcher-wrapper">
				<i class="co-control-url-autocomplete-spinner eicon-loading eicon-animation-spin" aria-hidden="true"></i>
				<input id="<?php echo $control_uid; ?>" class="co-control-tag-area co-input" data-setting="url" placeholder="{{ data.placeholder }}" />
				<input id="_ajax_linking_nonce" type="hidden" value="<?php echo wp_create_nonce( 'internal-linking' ); ?>" />
				<div class="co-control-url-more tooltip-target co-control-unit-1" data-tooltip="<?php echo __( 'Link Options', 'elementor' ); ?>">
					<i class="eicon-cog" aria-hidden="true"></i>
				</div>
			</div>
			<div class="co-control-url-more-options">
				<div class="co-control-url-option">
					<input id="<?php echo $is_external_control_uid; ?>" type="checkbox" class="co-control-url-option-input" data-setting="is_external">
					<label for="<?php echo $is_external_control_uid; ?>"><?php echo __( 'Open in new window', 'elementor' ); ?></label>
				</div>
				<div class="co-control-url-option">
					<input id="<?php echo $nofollow_control_uid; ?>" type="checkbox" class="co-control-url-option-input" data-setting="nofollow">
					<label for="<?php echo $nofollow_control_uid; ?>"><?php echo __( 'Add nofollow', 'elementor' ); ?></label>
				</div>
				<div class="co-control-url__custom-attributes">
					<label for="<?php echo $custom_attributes_uid; ?>" class="co-control-url__custom-attributes-label"><?php echo __( 'Custom Attributes', 'elementor' ); ?></label>
					<input type="text" id="<?php echo $custom_attributes_uid; ?>" class="co-control-unit-5" placeholder="key|value,key|value..." data-setting="custom_attributes">
				</div>
				<# if ( ( data.options && -1 !== data.options.indexOf( 'custom_attributes' ) ) && data.custom_attributes_description ) { #>
				<div class="co-control-field-description">{{{ data.custom_attributes_description }}}</div>
				<# } #>
			</div>
		</div>
		<# if ( data.description ) { #>
