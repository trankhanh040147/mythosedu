 ...
 *
 * Drag & Drop Layout preview handler class is responsible for initializing Drag & Drop Layout in
 * preview mode.
 *
 * @since 1.0.0
 */
class Preview extends App {
	 ...
	 * Init.
	 *
	 * Initialize Drag & Drop Layout preview mode.
	 *
	...
	public function init() {
		...
		if ( isset( $_GET['preview-debug'] ) ) {
			register_shutdown_function( function () {
				$e = error_get_last();
				if ( $e ) {
					echo '<div id="co-preview-debug-error"><pre>';
					echo $e['message'];
					echo '</pre></div>';
				}
			} );
		 ...
		 * Preview init.
		 *
		 * Fires on Drag & Drop Layout preview init, after Drag & Drop Layout preview has finished
	...
	public function is_preview_mode( $post_id = 0 ) {
		...
		if ( $post_id !== (int) $_GET['co-preview'] ) {
			return false;
		}
	...
	public function builder_wrapper( $content ) {
		if ( get_the_ID() === $this->post_id ) {
			...
			$attributes['class'] .= ' co-' . $this->post_id;
	...
	private function enqueue_styles() {
		 ...
		 *
		 * Fires after Drag & Drop Layout preview styles are enqueued.
		 *
		 * @since 1.0.0
		 */
	...
	private function enqueue_scripts() {
		 ...
		 *
		 * Fires after Drag & Drop Layout preview scripts are enqueued.
		 *
		 * @since 1.5.4
		 */
	...
	/**
	 * Drag & Drop Layout Preview footer scripts and styles.
	 *
	 ...
	 * Preview constructor.
	 *
	 * Initializing Drag & Drop Layout preview.
	 *
	 * @since 1.0.0
	 * @access public
	 */
