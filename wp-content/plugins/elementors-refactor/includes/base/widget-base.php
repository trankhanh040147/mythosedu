...
/**
 * Drag & Drop Layout widget base.
 *
 * An abstract class to register new Drag & Drop Layout widgets. It extended the
...
abstract class Widget_Base extends Element_Base {
	 ...
	 * posts widget in Drag & Drop Layout Pro. Default is true, the widget has content
	 * template.
	 *
	 * @access protected
	 *
	...
	public function __construct( $data = [], $args = null ) {
		...
		if ( $is_type_instance ) {
			 ...
			 * Widget skin init.
			 *
			 * Fires when Drag & Drop Layout widget is being initialized.
			 *
	...
	protected function get_html_wrapper_class() {
		return 'co-widget-' . $this->get_name();
	}
	...
	public function add_lightbox_data_to_image_link( $link_html, $id ) {
		...
		if ( Plugin::$instance->editor->is_edit_mode() ) {
			$this->add_render_attribute( 'link', 'class', 'co-clickable', true );
		}
	...
	public function add_lightbox_data_attributes( $element, $id = null, $lightbox_setting_key = null, $group_id = null, $overwrite = false ) {
		...
		if ( 'no' === $lightbox_setting_key ) {
			if ( $is_global_image_lightbox_enabled ) {
				$this->add_render_attribute( $element, 'data-co-open-lightbox', 'no' );
			}
		...
		$attributes['data-co-open-lightbox'] = 'yes';

		if ( $group_id ) {
			$attributes['data-co-lightbox-slideshow'] = $group_id;
		}
		...
		if ( $id ) {
			...
			if ( isset( $lightbox_image_attributes['title'] ) ) {
				$attributes['data-co-lightbox-title'] = $lightbox_image_attributes['title'];
			}

			if ( isset( $lightbox_image_attributes['description'] ) ) {
				$attributes['data-co-lightbox-description'] = $lightbox_image_attributes['description'];
			}
		}
	...
	public function render_content() {
		 ...
		 *
		 * Fires before Drag & Drop Layout widget is being rendered.
		 *
		 * @since 1.0.0
		 *
		 * @param Widget_Base $this The current widget.
	 ...
	 *
	 * Drag & Drop Layout saves the page content in a unique way, but it's not the way
	 ...
	 * also allows the site to keep working even if Drag & Drop Layout is deactivated.
	 *
	 ...
	 * an empty string. For example Drag & Drop Layout Pro Form Widget uses this method
	 * to return an empty string because there is no content to return. This way
	 * if Drag & Drop Layout Pro will be deactivated there won't be any form to display.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	...
	protected function add_inline_editing_attributes( $key, $toolbar = 'basic' ) {
		...
		$this->add_render_attribute( $key, [
			'class' => 'co-inline-editing',
			'data-co-setting-key' => $key,
		] );

		if ( 'basic' !== $toolbar ) {
			$this->add_render_attribute( $key, [
				'data-co-inline-editing-toolbar' => $toolbar,
			] );
		}
	}
