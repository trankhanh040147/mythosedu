...
/**
 * Drag & Drop Layout section element.
 *
 * Drag & Drop Layout section handler class is responsible for initializing the section
 * element.
 *
 * @since 1.0.0
 */
class Element_Section extends Element_Base {
	 ...
	 * the columns. But Drag & Drop Layout sets default values for predefined columns.
	 *
	 ...
	 *
	 * Note that Drag & Drop Layout sections have default section presets but the user
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'prefix_class' => 'co-',
				'hide_in_inner' => true,
				'description' => __( 'Stretch the section to the full width of the page using JS.', 'elementor' ) . sprintf( ' <a href="%1$s" target="_blank">%2$s</a>', '#', __( 'Learn more.', 'elementor' ) ),
				'render_type' => 'none',
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} > .co-container' => 'max-width: {{SIZE}}{{UNIT}};',
				],
				'condition' => [
					'layout' => [ 'boxed' ],
				],
		...
		$this->add_control(
			...
			[
				...
				'prefix_class' => 'co-section-height-',
				'hide_in_inner' => true,
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', 'vh', 'vw' ],
				'selectors' => [
					'{{WRAPPER}} > .co-container' => 'min-height: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} > .co-container:after' => 'content: ""; min-height: inherit;', // Hack for IE11
				],
				'condition' => [
					'height' => [ 'min-height' ],
				],
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} > .co-container > .co-row > .co-column > .co-column-wrap > .co-widget-wrap' => 'align-content: {{VALUE}}; align-items: {{VALUE}};',
				],
				// TODO: The following line is for BC since 2.7.0
				'prefix_class' => 'co-section-content-',
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} > .co-background-overlay',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} > .co-background-overlay' => 'opacity: {{SIZE}};',
				],
				'condition' => [
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} > .co-background-overlay' => 'mix-blend-mode: {{VALUE}}',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}}:hover > .co-background-overlay',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}}:hover > .co-background-overlay' => 'opacity: {{SIZE}};',
				],
				'condition' => [
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors' => [
					'{{WRAPPER}}, {{WRAPPER}} > .co-background-overlay' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors' => [
					'{{WRAPPER}}:hover, {{WRAPPER}}:hover > .co-background-overlay' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} > .co-background-overlay' => 'transition: background {{background_overlay_hover_transition.SIZE}}s, border-radius {{SIZE}}s, opacity {{background_overlay_hover_transition.SIZE}}s',
				],
			]
		);
		...
		foreach ( [
			...
			$this->add_control(
				...
				[
					...
					'selectors' => [
						"{{WRAPPER}} > .co-shape-$side .co-shape-fill" => 'fill: {{UNIT}};',
					],
				]
			);
			...
			$this->add_responsive_control(
				...
				[
					...
					'selectors' => [
						"{{WRAPPER}} > .co-shape-$side svg" => 'width: calc({{SIZE}}{{UNIT}} + 1.3px)',
					],
				]
			);
			...
			$this->add_responsive_control(
				...
				[
					...
					'selectors' => [
						"{{WRAPPER}} > .co-shape-$side svg" => 'height: {{SIZE}}{{UNIT}};',
					],
				]
			);
			...
			$this->add_control(
				...
				[
					...
					'selectors' => [
						"{{WRAPPER}} > .co-shape-$side svg" => 'transform: translateX(-50%) rotateY(180deg)',
					],
				]
			);
			...
			$this->add_control(
				...
				[
					...
					'type' => Controls_Manager::SWITCHER,
					'selectors' => [
						"{{WRAPPER}} > .co-shape-$side" => 'z-index: 2; pointer-events: none',
					],
					'condition' => [
						"shape_divider_$side!" => '',
					],
		...
		if ( in_array( Schemes\Color::get_type(), Schemes\Manager::get_enabled_schemes(), true ) ) {
			$this->add_control(
				...
				[
					...
					'content_classes' => 'co-panel-alert co-panel-alert-danger',
				]
			);
		}
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .co-heading-title' => 'color: {{VALUE}};',
				],
				'separator' => 'none',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} > .co-container' => 'text-align: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'style_transfer' => false,
				'classes' => 'co-control-direction-ltr',
			]
		);
	...
	protected function content_template() {
		...
		if ( settings.background_video_link ) {
			...
			view.addRenderAttribute( 'background-video-container', 'class', 'co-background-video-container' );

			if ( ! settings.background_play_on_mobile ) {
				view.addRenderAttribute( 'background-video-container', 'class', 'co-hidden-phone' );
			}
		#>
			<div {{{ view.getRenderAttributeString( 'background-video-container' ) }}}>
				<div class="co-background-video-embed"></div>
				<video class="co-background-video-hosted co-html5-video" {{ videoAttributes }}></video>
			</div>
		<# } #>
		<div class="co-background-overlay"></div>
		<div class="co-shape co-shape-top"></div>
		<div class="co-shape co-shape-bottom"></div>
		<div class="co-container co-column-gap-{{ settings.gap }}">
			<div class="co-row"></div>
		</div>
		<?php
	}
		...
		<<?php echo esc_html( $this->get_html_tag() ); ?> <?php $this->print_render_attribute_string( '_wrapper' ); ?>>
			...
			if ( 'video' === $settings['background_background'] ) :
				if ( $settings['background_video_link'] ) :
					...
					$this->add_render_attribute( 'background-video-container', 'class', 'co-background-video-container' );

					if ( ! $settings['background_play_on_mobile'] ) {
						$this->add_render_attribute( 'background-video-container', 'class', 'co-hidden-phone' );
					}
					?>
					<div <?php echo $this->get_render_attribute_string( 'background-video-container' ); ?>>
						<?php if ( $video_properties ) : ?>
							...
							?>
							<video class="co-background-video-hosted co-html5-video" <?php echo $video_tag_attributes; ?>></video>
						<?php endif; ?>
					</div>
					<?php
				endif;
			...
			?>
			<div class="co-container co-column-gap-<?php echo esc_attr( $settings['gap'] ); ?>">
				<div class="co-row">
		<?php
	}
	...
	private function print_shape_divider( $side ) {
		...
		?>
		<div class="co-shape co-shape-<?php echo esc_attr( $side ); ?>" data-negative="<?php echo var_export( $negative ); ?>">
