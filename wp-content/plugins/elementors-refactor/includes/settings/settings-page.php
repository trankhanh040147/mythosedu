...
/**
 * Drag & Drop Layout settings page.
 *
...
abstract class Settings_Page {
	...
	public function display_settings_page() {
		...
		<div class="wrap">
			...
			<div id="co-settings-tabs-wrapper" class="nav-tab-wrapper">
				<?php
				foreach ( $tabs as $tab_id => $tab ) {
					...
					echo "<a id='co-settings-tab-{$tab_id}' class='nav-tab{$active_class}' href='#tab-{$tab_id}'>{$tab['label']}</a>";
				}
				?>
			</div>
			<form id="co-settings-form" method="post" action="options.php">
				...
				foreach ( $tabs as $tab_id => $tab ) {
					...
					if ( 'general' === $tab_id ) {
						$active_class = ' co-active';
					}

					echo "<div id='tab-{$tab_id}' class='co-settings-form-page{$active_class}'>";
	...
	private function ensure_tabs() {
		if ( null === $this->tabs ) {
			 ...
			 *
			 * Fires after the settings are created in Drag & Drop Layout admin page.
			 *
