...
/**
 * Drag & Drop Layout "Tools" page in WordPress Dashboard.
 *
...
class Tools extends Settings_Page {
	...
	/**
	 * Settings page ID for Drag & Drop Layout tools.
	 */
	const PAGE_ID = 'co-tools';

	/**
	 * Register admin menu.
	 *
	 * Add new Drag & Drop Layout Tools admin menu.
	 *
	 ...
	 * updates all the Drag & Drop Layout data.
	 *
	...
	/**
	 * Drag & Drop Layout version rollback.
	 *
	 * Rollback to previous Drag & Drop Layout version.
	 *
	 ...
	 *
	 * Initializing Drag & Drop Layout "Tools" page.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	...
	protected function create_tabs() {
		$rollback_html = '<select class="co-rollback-select">';
					...
					'tools' => [
						'fields' => [
							'clear_cache' => [
								...
								'field_args' => [
									'type' => 'raw_html',
									'html' => sprintf( '<button data-nonce="%s" class="button co-button-spinner" id="co-clear-cache-button">%s</button>', wp_create_nonce( 'elementor_clear_cache' ), __( 'Regenerate Files', 'elementor' ) ),
									'desc' => __( 'Styles set in Drag & Drop Layout are saved in CSS files in the uploads folder. Recreate those files, according to the most recent settings.', 'elementor' ),
								],
							],
							'reset_api_data' => [
								...
								'field_args' => [
									'type' => 'raw_html',
									'html' => sprintf( '<button data-nonce="%s" class="button co-button-spinner" id="co-library-sync-button">%s</button>', wp_create_nonce( 'elementor_reset_library' ), __( 'Sync Library', 'elementor' ) ),
									'desc' => __( 'Drag & Drop Layout Library automatically updates on a daily basis. You can also manually update it by clicking on the sync button.', 'elementor' ),
								],
							],
						],
					],
			...
			'versions' => [
				...
				'sections' => [
					'rollback' => [
						...
						'callback' => function() {
							$intro_text = sprintf(
								...
								__( 'Experiencing an issue with Drag & Drop Layout version %s? Rollback to a previous version before the issue appeared.', 'elementor' ),
								ELEMENTOR_VERSION
							);
						...
						'fields' => [
							'rollback' => [
								...
								'field_args' => [
									'type' => 'raw_html',
									'html' => sprintf(
										$rollback_html . '<a data-placeholder-text="' . __( 'Reinstall', 'elementor' ) . ' v{VERSION}" href="#" data-placeholder-url="%s" class="button co-button-spinner co-rollback-button">%s</a>',
					...
					'beta' => [
						...
						'callback' => function() {
							$intro_text = __( 'Turn-on Beta Tester, to get notified when a new beta version of Drag & Drop Layout or Drag & Drop Layout Pro is available. The Beta version will not install automatically. You always have the option to ignore it.', 'elementor' );
							$intro_text = '<p>' . $intro_text . '</p>';
