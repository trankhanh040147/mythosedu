...
/**
 * Drag & Drop Layout testimonial widget.
 *
 * Drag & Drop Layout widget that displays customer testimonials that show social proof.
 *
 * @since 1.0.0
 */
class Widget_Testimonial extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .co-testimonial-content' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .co-testimonial-content',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .co-testimonial-wrapper .co-testimonial-image img' => 'width: {{SIZE}}{{UNIT}};height: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				'name' => 'image_border',
				'selector' => '{{WRAPPER}} .co-testimonial-wrapper .co-testimonial-image img',
				'separator' => 'before',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors' => [
					'{{WRAPPER}} .co-testimonial-wrapper .co-testimonial-image img' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .co-testimonial-name' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .co-testimonial-name',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .co-testimonial-job' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .co-testimonial-job',
			]
		);
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'co-testimonial-wrapper' );

		if ( $settings['testimonial_alignment'] ) {
			$this->add_render_attribute( 'wrapper', 'class', 'co-testimonial-text-align-' . $settings['testimonial_alignment'] );
		}

		$this->add_render_attribute( 'meta', 'class', 'co-testimonial-meta' );

		if ( $settings['testimonial_image']['url'] ) {
			$this->add_render_attribute( 'meta', 'class', 'co-has-image' );
		}

		if ( $settings['testimonial_image_position'] ) {
			$this->add_render_attribute( 'meta', 'class', 'co-testimonial-image-position-' . $settings['testimonial_image_position'] );
		}
		...
		<div <?php echo $this->get_render_attribute_string( 'wrapper' ); ?>>
			<?php
			if ( $has_content ) :
				$this->add_render_attribute( 'testimonial_content', 'class', 'co-testimonial-content' );
			...
			<div <?php echo $this->get_render_attribute_string( 'meta' ); ?>>
				<div class="co-testimonial-meta-inner">
					<?php if ( $has_image ) : ?>
						<div class="co-testimonial-image">
							<?php
					...
					<div class="co-testimonial-details">
						<?php
						if ( $has_name ) :
							$this->add_render_attribute( 'testimonial_name', 'class', 'co-testimonial-name' );
						...
						<?php
						if ( $has_job ) :
							$this->add_render_attribute( 'testimonial_job', 'class', 'co-testimonial-job' );
	...
	protected function content_template() {
		...
		var testimonial_alignment = settings.testimonial_alignment ? ' co-testimonial-text-align-' + settings.testimonial_alignment : '';
		var testimonial_image_position = settings.testimonial_image_position ? ' co-testimonial-image-position-' + settings.testimonial_image_position : '';
		#>
		<div class="co-testimonial-wrapper{{ testimonial_alignment }}">
			<# if ( '' !== settings.testimonial_content ) {
				view.addRenderAttribute( 'testimonial_content', 'class', 'co-testimonial-content' );
			...
			<# } #>
			<div class="co-testimonial-meta{{ hasImage }}{{ testimonial_image_position }}">
				<div class="co-testimonial-meta-inner">
					<# if ( imageUrl ) { #>
						<div class="co-testimonial-image">{{{ imageHtml }}}</div>
					<# } #>
					<div class="co-testimonial-details">
	...
	protected function render_testimonial_description() {
		...
		if ( '' !== settings.testimonial_name ) {
			view.addRenderAttribute( 'testimonial_name', 'class', 'co-testimonial-name' );
		...
		if ( '' !== settings.testimonial_job ) {
			view.addRenderAttribute( 'testimonial_job', 'class', 'co-testimonial-job' );
