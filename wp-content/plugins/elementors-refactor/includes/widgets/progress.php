...
/**
 * Drag & Drop Layout progress widget.
 *
 * Drag & Drop Layout widget that displays an escalating progress bar.
 *
 * @since 1.0.0
 */
class Widget_Progress extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .co-progress-wrapper .co-progress-bar' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .co-progress-wrapper' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::SLIDER,
				'selectors' => [
					'{{WRAPPER}} .co-progress-bar' => 'height: {{SIZE}}{{UNIT}}; line-height: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors' => [
					'{{WRAPPER}} .co-progress-wrapper' => 'border-radius: {{SIZE}}{{UNIT}}; overflow: hidden;',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .co-progress-bar' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .co-progress-bar',
				'exclude' => [
					'line_height',
				],
			]
		...
		<div <?php echo $this->get_render_attribute_string( 'wrapper' ); ?>>
			<div <?php echo $this->get_render_attribute_string( 'progress-bar' ); ?>>
				...
				<?php if ( 'hide' !== $settings['display_percentage'] ) { ?>
					<span class="co-progress-percentage"><?php echo $progress_percentage; ?>%</span>
				<?php } ?>
			</div>
		</div>
		<?php
	...
	protected function content_template() {
		...
		view.addRenderAttribute( 'progressWrapper', {
			'class': [ 'co-progress-wrapper', 'progress-' + settings.progress_type ],
			'role': 'progressbar',
			'aria-valuemin': '0',
			'aria-valuemax': '100',
			'aria-valuenow': progress_percentage,
		...
		<div {{{ view.getRenderAttributeString( 'progressWrapper' ) }}}>
			<div class="co-progress-bar" data-max="{{ progress_percentage }}">
				<span {{{ view.getRenderAttributeString( 'inner_text' ) }}}>{{{ settings.inner_text }}}</span>
				<# if ( 'hide' !== settings.display_percentage ) { #>
					<span class="co-progress-percentage">{{{ progress_percentage }}}%</span>
				<# } #>
			</div>
		</div>
		<?php
