...
/**
 * Drag & Drop Layout star rating widget.
 *
 * Drag & Drop Layout widget that displays star rating.
 *
 * @since 2.3.0
 */
class Widget_Star_Rating extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'prefix_class' => 'co-star-rating%s--align-',
				'selectors' => [
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .co-star-rating__title' => 'color: {{VALUE}}',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .co-star-rating__title',
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'body:not(.rtl) {{WRAPPER}}:not(.co-star-rating--align-justify) .co-star-rating__title' => 'margin-right: {{SIZE}}{{UNIT}}',
					'body.rtl {{WRAPPER}}:not(.co-star-rating--align-justify) .co-star-rating__title' => 'margin-left: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .co-star-rating' => 'font-size: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'body:not(.rtl) {{WRAPPER}} .co-star-rating i:not(:last-of-type)' => 'margin-right: {{SIZE}}{{UNIT}}',
					'body.rtl {{WRAPPER}} .co-star-rating i:not(:last-of-type)' => 'margin-left: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .co-star-rating i:before' => 'color: {{VALUE}}',
				],
				'separator' => 'before',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .co-star-rating i' => 'color: {{VALUE}}',
				],
			]
		);
	...
	protected function render_stars( $icon ) {
		...
		for ( $stars = 1.0; $stars <= $rating_data[1]; $stars++ ) {
			if ( $stars <= $floored_rating ) {
				$stars_html .= '<i class="co-star-full">' . $icon . '</i>';
			} elseif ( $floored_rating + 1 === $stars && $rating !== $floored_rating ) {
				$stars_html .= '<i class="co-star-' . ( $rating - $floored_rating ) * 10 . '">' . $icon . '</i>';
			} else {
				$stars_html .= '<i class="co-star-empty">' . $icon . '</i>';
			}
		}
	...
	protected function render() {
		...
		$schema_rating = '<span itemprop="ratingValue" class="co-screen-only">' . $textual_rating . '</span>';
		$stars_element = '<div ' . $this->get_render_attribute_string( 'icon_wrapper' ) . '>' . $this->render_stars( $icon ) . ' ' . $schema_rating . '</div>';
		?>

		<div class="co-star-rating__wrapper">
			<?php if ( ! Utils::is_empty( $settings['title'] ) ) : ?>
				<div class="co-star-rating__title"><?php echo $settings['title']; ?></div>
			<?php endif; ?>
	...
	protected function content_template() {
		...
		<#
			...
			renderStars = function( icon ) {
				...
				for ( var stars = 1; stars <= ratingData[1]; stars++ ) {
					if ( stars <= flooredRating  ) {
						starsHtml += '<i class="co-star-full">' + icon + '</i>';
					} else if ( flooredRating + 1 === stars && rating !== flooredRating ) {
						starsHtml += '<i class="co-star-' + ( rating - flooredRating ).toFixed( 1 ) * 10 + '">' + icon + '</i>';
					} else {
						starsHtml += '<i class="co-star-empty">' + icon + '</i>';
					}
				}
			...
			view.addRenderAttribute( 'iconWrapper', 'class', 'co-star-rating' );
		...
		<div class="co-star-rating__wrapper">
			<# if ( ! _.isEmpty( settings.title ) ) { #>
				<div class="co-star-rating__title">{{ settings.title }}</div>
			<# } #>
			<div {{{ view.getRenderAttributeString( 'iconWrapper' ) }}} >
				{{{ stars }}}
				<span itemprop="ratingValue" class="co-screen-only">{{ textualRating }}</span>
			</div>
		</div>
