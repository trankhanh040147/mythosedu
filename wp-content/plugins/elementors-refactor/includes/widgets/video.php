...
/**
 * Drag & Drop Layout video widget.
 *
 * Drag & Drop Layout widget that displays a video player.
 *
 * @since 1.0.0
 */
class Widget_Video extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => 'https://www.youtube.com/watch?v=fFdoohwUy0s',
				'label_block' => true,
				'condition' => [
					'video_type' => 'youtube',
				],
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .co-custom-embed-play i' => 'color: {{VALUE}}',
				],
				'condition' => [
					'show_image_overlay' => 'yes',
					'show_play_icon' => 'yes',
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .co-custom-embed-play i' => 'font-size: {{SIZE}}{{UNIT}}',
				],
				'condition' => [
					'show_image_overlay' => 'yes',
					'show_play_icon' => 'yes',
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .co-custom-embed-play i',
				'fields_options' => [
					'text_shadow_type' => [
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'#co-lightbox-{{ID}}' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'#co-lightbox-{{ID}} .dialog-lightbox-close-button' => 'color: {{VALUE}}',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'#co-lightbox-{{ID}} .dialog-lightbox-close-button:hover' => 'color: {{VALUE}}',
				],
				'separator' => 'after',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'(desktop+)#co-lightbox-{{ID}} .co-video-container' => 'width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'#co-lightbox-{{ID}} .co-video-container' => '{{VALUE}}; transform: translateX(-50%);',
				],
	...
	protected function render() {
		...
		$this->add_render_attribute( 'video-wrapper', 'class', 'co-wrapper' );

		if ( ! $settings['lightbox'] ) {
			$this->add_render_attribute( 'video-wrapper', 'class', 'co-fit-aspect-ratio' );
		}

		$this->add_render_attribute( 'video-wrapper', 'class', 'co-open-' . ( $settings['lightbox'] ? 'lightbox' : 'inline' ) );
		?>
		<div <?php echo $this->get_render_attribute_string( 'video-wrapper' ); ?>>
			...
			if ( $this->has_image_overlay() ) {
				$this->add_render_attribute( 'image-overlay', 'class', 'co-custom-embed-image-overlay' );
				...
				if ( $settings['lightbox'] ) {
					...
					$this->add_render_attribute( 'image-overlay', [
						'data-co-open-lightbox' => 'yes',
						'data-co-lightbox' => wp_json_encode( $lightbox_options ),
					] );
				...
				<div <?php echo $this->get_render_attribute_string( 'image-overlay' ); ?>>
					...
					<?php if ( 'yes' === $settings['show_play_icon'] ) : ?>
						<div class="co-custom-embed-play" role="button">
							<i class="eicon-play" aria-hidden="true"></i>
							<span class="co-screen-only"><?php echo __( 'Play Video', 'elementor' ); ?></span>
						</div>
					<?php endif; ?>
				</div>
			<?php } ?>
	...
	private function render_hosted_video() {
		...
		?>
		<video class="co-video" src="<?php echo esc_url( $video_url ); ?>" <?php echo Utils::render_html_attributes( $video_params ); ?>></video>
		<?php
	}
}
