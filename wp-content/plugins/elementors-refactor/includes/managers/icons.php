...
/**
 * Drag & Drop Layout icons manager.
 *
 * Drag & Drop Layout icons manager handler class
 *
 * @since 2.4.0
 */
class Icons_Manager {
			...
			if ( ! empty( $icon_type['enqueue'] ) ) {
				foreach ( (array) $icon_type['enqueue'] as $font_css_url ) {
					if ( ! in_array( $font_css_url, array_keys( $shared_styles ) ) ) {
						$style_handle = 'co-icons-shared-' . count( $shared_styles );
						wp_register_style(
							$style_handle,
							$font_css_url,
							[],
	...
	public function register_admin_tools_settings( Tools $settings ) {
		...
		$settings->add_section( 'fontawesome4_migration', 'fontawesome4_migration', [
			'callback' => function() {
				...
				__( 'By upgrading, whenever you edit a page containing a Font Awesome 4 icon, Drag & Drop Layout will convert it to the new Font Awesome 5 icon.', 'elementor' ) .
				'</p><p><strong>' .
