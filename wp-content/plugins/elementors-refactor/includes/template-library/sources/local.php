...
/**
 * Drag & Drop Layout template library local source.
 *
 * Drag & Drop Layout template library local source handler class is responsible for
 * handling local Drag & Drop Layout templates saved by the user locally on his site.
 *
 * @since 1.0.0
 */
class Source_Local extends Source_Base {
	...
	/**
	 * Drag & Drop Layout template-library post-type slug.
	 */
	const CPT = 'elementor_library';

	/**
	 * Drag & Drop Layout template-library taxonomy slug.
	 */
	const TAXONOMY_TYPE_SLUG = 'elementor_library_type';

	/**
	 * Drag & Drop Layout template-library category slug.
	 */
	const TAXONOMY_CATEGORY_SLUG = 'elementor_library_category';

	/**
	 * Drag & Drop Layout template-library meta key.
	 * @deprecated 2.3.0 Use \Elementor\Core\Base\Document::TYPE_META_KEY instead
	 */
	const TYPE_META_KEY = '_elementor_template_type';

	/**
	 * Drag & Drop Layout template-library temporary files folder.
	 */
	const TEMP_FILES_DIR = 'elementor/tmp';

	/**
	 * Drag & Drop Layout template-library bulk export action name.
	 */
	...
	public function save_item( $template_data ) {
		 ...
		 *
		 * Fires after Drag & Drop Layout template library was saved.
		 *
		 * @since 1.0.1
		 *
		 ...
		 *
		 * Fires after Drag & Drop Layout template library was updated.
		 *
		 * @since 1.0.1
		 *
		 * @param int   $template_id   The ID of the template.
	...
	public function export_multiple_templates( array $template_ids ) {
		...
		$zip_archive_filename = 'co-templates-' . gmdate( 'Y-m-d' ) . '.zip';
	...
	public function admin_import_template_form() {
		...
		?>
		<div id="co-hidden-area">
			<a id="co-import-template-trigger" class="page-title-action"><?php echo __( 'Import Templates', 'elementor' ); ?></a>
			<div id="co-import-template-area">
				<div id="co-import-template-title"><?php echo __( 'Choose an Drag & Drop Layout template JSON file or a .zip archive of Drag & Drop Layout templates, and add them to the list of templates available in your library.', 'elementor' ); ?></div>
				<form id="co-import-template-form" method="post" action="<?php echo admin_url( 'admin-ajax.php' ); ?>" enctype="multipart/form-data">
					...
					<fieldset id="co-import-template-form-inputs">
	...
	/**
	 * Remove Drag & Drop Layout post state.
	 *
	...
	public function admin_print_tabs( $views ) {
		...
		?>
		<div id="co-template-library-tabs-wrapper" class="nav-tab-wrapper">
	...
	public function maybe_render_blank_state( $which ) {
		...
		<div class="co-template_library-blank_state">
			<div class="co-blank_state">
				...
				<a id="co-template-library-add-new" class="co-button co-button-success" href="<?php esc_url( Utils::get_pro_link( '#' ) ); ?>">
					<?php
					/* translators: %s: Template type label. */
	...
	private function prepare_template_export( $template_id ) {
		...
		return [
			'name' => 'co-' . $template_id . '-' . gmdate( 'Y-m-d' ) . '.json',
