...
/**
 * Drag & Drop Layout frontend.
 *
 * Drag & Drop Layout frontend handler class is responsible for initializing Drag & Drop Layout in
 * the frontend.
 *
 * @since 1.0.0
 */
class Frontend extends App {
	 ...
	 * @access private
	 *
	 * @var bool Whether Drag & Drop Layout is being used. Default is false.
	 */
	 ...
	 * conflicted with Drag & Drop Layout while Drag & Drop Layout process the content.
	 *
	 * @since 1.0.0
	 * @access private
	 *
	 ...
	 *
	 * Initializing Drag & Drop Layout front end. Make sure we are not in admin, not and
	 * redirect from old URL structure of Drag & Drop Layout editor.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	 ...
	 * Init.
	 *
	 * Initialize Drag & Drop Layout front end. Hooks the needed actions to run Elementor
	...
	public function init() {
		...
		// Add Edit with the Drag & Drop Layout in Admin Bar.
		add_action( 'admin_bar_menu', [ $this, 'add_menu_in_admin_bar' ], 200 );

		// Detect Drag & Drop Layout documents via their css that printed before the Admin Bar.
	 ...
	 *
	 * When the Drag & Drop Layout generated content rendered, we remove the filter to prevent multiple
	 * accuracies. This way we make sure Drag & Drop Layout renders the content only once.
	 *
	 * @since 1.8.0
	 * @access public
	 */
	...
	public function register_scripts() {
		 ...
		 *
		 * Fires before Drag & Drop Layout frontend scripts are registered.
		 *
		 * @since 1.2.1
		 */
		 ...
		 *
		 * Fires after Drag & Drop Layout frontend scripts are registered.
		 *
		 * @since 1.2.1
		 */
	...
	public function register_styles() {
		 ...
		 *
		 * Fires before Drag & Drop Layout frontend styles are registered.
		 *
		 * @since 1.2.0
		 */
		...
		wp_register_style(
			'co-icons',
			$this->get_css_assets_url( 'co-icons', 'assets/lib/eicons/css/' ),
			[],
			'5.6.2'
		);
		 ...
		 *
		 * Fires after Drag & Drop Layout frontend styles are registered.
		 *
		 * @since 1.2.0
		 */
	...
	public function enqueue_scripts() {
		 ...
		 *
		 * Fires before Drag & Drop Layout frontend scripts are enqueued.
		 *
		 * @since 1.0.0
		 */
		do_action( 'elementor/frontend/before_enqueue_scripts' );

		wp_enqueue_script( 'co-frontend' );
		 ...
		 *
		 * Fires after Drag & Drop Layout frontend scripts are enqueued.
		 *
		 * @since 1.0.0
		 */
	...
	public function enqueue_styles() {
		 ...
		 *
		 * Fires before Drag & Drop Layout frontend styles are enqueued.
		 *
		 * @since 1.0.0
		 */
		do_action( 'elementor/frontend/before_enqueue_styles' );

		wp_enqueue_style( 'co-icons' );
		wp_enqueue_style( 'co-animations' );
		wp_enqueue_style( 'co-frontend' );

		/**
		 * After frontend styles enqueued.
		 *
		 * Fires after Drag & Drop Layout frontend styles are enqueued.
		 *
		 * @since 1.0.0
		 */
	...
	/**
	 * Drag & Drop Layout footer scripts and styles.
	 *
		...
		foreach ( $this->fonts_to_enqueue as $key => $font ) {
			...
			switch ( $font_type ) {
				...
				default:
					 ...
					 * Print font links.
					 *
					 * Fires when Drag & Drop Layout frontend fonts are printed on the HEAD tag.
					 *
	...
	private function enqueue_icon_fonts() {
		...
		foreach ( $this->icon_fonts_to_enqueue as $icon_type => $css_url ) {
			wp_enqueue_style( 'co-icons-' . $icon_type );
	 ...
	 *
	 * Used to apply the Drag & Drop Layout page editor on the post content.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 *
	 * Used to render and return the post content with all the Drag & Drop Layout elements.
	 *
	...
	/**
	 * Add Drag & Drop Layout menu to admin bar.
	 *
	...
	public function add_menu_in_admin_bar( \WP_Admin_Bar $wp_admin_bar ) {
		...
		$menu_args = [
			...
			'title' => __( 'Edit with Drag & Drop Layout', 'elementor' ),
		];
		...
		foreach ( $this->admin_bar_edit_documents as $document ) {
			$wp_admin_bar->add_menu( [
				...
				'title' => sprintf( '<span class="co-edit-link-title">%s</span><span class="co-edit-link-type">%s</span>', $document->get_post()->post_title, $document::get_title() ),
	...
	public function get_builder_content_for_display( $post_id, $with_css = false ) {
		...
		if ( get_the_ID() === (int) $post_id ) {
			...
			if ( $editor->is_edit_mode() ) {
				$content = '<div class="co-alert co-alert-danger">' . __( 'Invalid Data: The Template ID cannot be the same as the currently edited template. Please choose a different one.', 'elementor' ) . '</div>';
			}
	...
	public function create_action_hash( $action, array $settings = [] ) {
		return rawurlencode( sprintf( '#co-action:action=%1$s&settings=%2$s', $action, base64_encode( wp_json_encode( $settings ) ) ) );
	}
	...
	private function process_more_tag( $content ) {
		...
		$more_link = apply_filters( 'the_content_more_link', sprintf( ' <a href="%s#more-%s" class="more-link co-more-link">%s</a>', get_permalink(), $post->ID, $more_link_text ), $more_link_text );
