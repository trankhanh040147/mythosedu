...
class Feedback extends Module {
	...
	public function enqueue_feedback_dialog_scripts() {
		...
		wp_enqueue_script( 'co-admin-feedback' );
	}
	...
	public function print_deactivate_feedback_dialog() {
		$deactivate_reasons = [
			...
			'elementor_pro' => [
				'title' => __( 'I have Drag & Drop Layout Pro', 'elementor' ),
				'input_placeholder' => '',
				'alert' => __( 'Wait! Don\'t deactivate Elementor. You have to activate both Drag & Drop Layout and Drag & Drop Layout Pro in order for the plugin to work.', 'elementor' ),
			],
			'other' => [
		...
		?>
		<div id="co-deactivate-feedback-dialog-wrapper">
			<div id="co-deactivate-feedback-dialog-header">
				<i class="eicon-co-square" aria-hidden="true"></i>
				<span id="co-deactivate-feedback-dialog-header-title"><?php echo __( 'Quick Feedback', 'elementor' ); ?></span>
			</div>
			<form id="co-deactivate-feedback-dialog-form" method="post">
				<?php
				wp_nonce_field( '_elementor_deactivate_feedback_nonce' );
				?>
				<input type="hidden" name="action" value="elementor_deactivate_feedback" />

				<div id="co-deactivate-feedback-dialog-form-caption"><?php echo __( 'If you have a moment, please share why you are deactivating Elementor:', 'elementor' ); ?></div>
				<div id="co-deactivate-feedback-dialog-form-body">
					<?php foreach ( $deactivate_reasons as $reason_key => $reason ) : ?>
						<div class="co-deactivate-feedback-dialog-input-wrapper">
							<input id="co-deactivate-feedback-<?php echo esc_attr( $reason_key ); ?>" class="co-deactivate-feedback-dialog-input" type="radio" name="reason_key" value="<?php echo esc_attr( $reason_key ); ?>" />
							<label for="co-deactivate-feedback-<?php echo esc_attr( $reason_key ); ?>" class="co-deactivate-feedback-dialog-label"><?php echo esc_html( $reason['title'] ); ?></label>
							<?php if ( ! empty( $reason['input_placeholder'] ) ) : ?>
								<input class="co-feedback-text" type="text" name="reason_<?php echo esc_attr( $reason_key ); ?>" placeholder="<?php echo esc_attr( $reason['input_placeholder'] ); ?>" />
							<?php endif; ?>
							<?php if ( ! empty( $reason['alert'] ) ) : ?>
								<div class="co-feedback-text"><?php echo esc_html( $reason['alert'] ); ?></div>
							<?php endif; ?>
						</div>
					<?php endforeach; ?>
				</div>
	 ...
	 *
	 * Send the user feedback when Drag & Drop Layout is deactivated.
	 *
