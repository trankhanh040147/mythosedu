...
class Admin extends App {
	...
	public function maybe_redirect_to_getting_started() {
		...
		wp_safe_redirect( admin_url( 'admin.php?page=co-getting-started' ) );
	 ...
	 * the user to switch from the native WordPress editor to Drag & Drop Layout builder.
	 *
	...
	public function print_switch_mode_button( $post ) {
		...
		<div id="co-switch-mode">
			<input id="co-switch-mode-input" type="hidden" name="_elementor_post_mode" value="<?php echo $document->is_built_with_elementor(); ?>" />
			<button id="co-switch-mode-button" type="button" class="button button-primary button-hero">
				<span class="co-switch-mode-on">
		...
		<div id="co-editor">
			<a id="co-go-to-edit-page-link" href="<?php echo $document->get_edit_url(); ?>">
	...
	/**
	 * Add Drag & Drop Layout post state.
	 *
	...
	public function add_elementor_post_state( $post_states, $post ) {
		if ( User::is_current_user_can_edit( $post->ID ) && Plugin::$instance->db->is_built_with_elementor( $post->ID ) ) {
			$post_states['elementor'] = __( 'Drag & Drop Layout', 'elementor' );
		}
	...
	public function body_status_classes( $classes ) {
		...
		if ( in_array( $pagenow, [ 'post.php', 'post-new.php' ], true ) && Utils::is_post_support() ) {
			...
			$mode_class = Plugin::$instance->db->is_built_with_elementor( $post->ID ) ? 'co-editor-active' : 'co-editor-inactive';
	...
	public function plugin_action_links( $links ) {
		...
		$links['go_pro'] = sprintf( '<a href="%1$s" target="_blank" class="co-plugins-gopro">%2$s</a>', Utils::get_pro_link( '#' ), __( 'Go Pro', 'elementor' ) );
	...
	public function plugin_row_meta( $plugin_meta, $plugin_file ) {
		if ( ELEMENTOR_PLUGIN_BASE === $plugin_file ) {
			$row_meta = [
				'docs' => '<a href="#" aria-label="' . esc_attr( __( 'View Drag & Drop Layout Documentation', 'elementor' ) ) . '" target="_blank">' . __( 'Docs & FAQs', 'elementor' ) . '</a>',
				'ideo' => '<a href="#" aria-label="' . esc_attr( __( 'View Drag & Drop Layout Video Tutorials', 'elementor' ) ) . '" target="_blank">' . __( 'Video Tutorials', 'elementor' ) . '</a>',
			];
	...
	public function admin_footer_text( $footer_text ) {
		...
		if ( $is_elementor_screen ) {
			$footer_text = sprintf(
				...
				'<strong>' . __( 'Drag & Drop Layout', 'elementor' ) . '</strong>',
	 ...
	 *
	 * Adds a new Drag & Drop Layout widgets to WordPress dashboard.
	 *
	...
	public function register_dashboard_widgets() {
		wp_add_dashboard_widget( 'e-dashboard-overview', __( 'Drag & Drop Layout Overview', 'elementor' ), [ $this, 'elementor_dashboard_overview_widget' ] );
	...
	/**
	 * Drag & Drop Layout dashboard widget.
	 *
	 * Displays the Drag & Drop Layout dashboard widget.
	 *
		...
		<div class="e-dashboard-widget">
			<div class="e-overview__header">
				...
				<div class="e-overview__versions">
					<span class="e-overview__version"><?php echo __( 'Drag & Drop Layout', 'elementor' ); ?> v<?php echo ELEMENTOR_VERSION; ?></span>
					<?php
					/**
					 * Drag & Drop Layout dashboard widget after the version.
					 *
					 * Fires after Drag & Drop Layout version display in the dashboard widget.
					 *
					 * @since 1.9.0
					 */
	...
	private function get_dashboard_overview_widget_footer_actions() {
		 ...
		 *
		 * Filters the additions actions displayed in Drag & Drop Layout dashboard widget.
		 *
		 * Developers can add new action links to Drag & Drop Layout dashboard widget
		 * footer using this filter.
		 *
		 * @since 1.9.0
		 *
		 * @param array $additions_actions Drag & Drop Layout dashboard widget footer actions.
		 */
	 ...
	 *
	 * When a new post action is fired the title is set to 'Drag & Drop Layout' and the post ID.
	 *
	 ...
	 * Admin constructor.
	 *
	 * Initializing Drag & Drop Layout in WordPress admin.
	 *
	 * @since 1.0.0
	 * @access public
	 */
