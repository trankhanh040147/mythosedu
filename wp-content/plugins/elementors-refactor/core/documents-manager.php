...
/**
 * Drag & Drop Layout documents manager.
 *
 * Drag & Drop Layout documents manager handler class is responsible for registering and
 * managing Drag & Drop Layout documents.
 *
 * @since 2.0.0
 */
class Documents_Manager {
	 ...
	 *
	 * Initializing the Drag & Drop Layout documents manager.
	 *
	 * @since 2.0.0
	 * @access public
	 */
	...
	public function create( $type, $post_data = [], $meta_data = [] ) {
		...
		if ( empty( $post_data['post_title'] ) ) {
			$post_data['post_title'] = __( 'Drag & Drop Layout', 'elementor' );
			if ( 'post' !== $type ) {
				$post_data['post_title'] = sprintf(
					/* translators: %s: Document title */
					__( 'Title %s', 'elementor' ),
	...
	private function register_types() {
		if ( ! did_action( 'elementor/documents/register' ) ) {
			/**
			 * Register Drag & Drop Layout documents.
			 *
			 * @since 2.0.0
			 *
