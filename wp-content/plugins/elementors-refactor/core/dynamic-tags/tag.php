...
/**
 * Drag & Drop Layout tag.
 *
 * An abstract class to register new Drag & Drop Layout tag.
 *
 * @since 2.0.0
 * @abstract
 */
abstract class Tag extends Base_Tag {
	...
	public function get_content( array $options = [] ) {
		...
		if ( ! Utils::is_empty( $value ) ) {
			// TODO: fix spaces in `before`/`after` if WRAPPED_TAG ( conflicted with .co-tag { display: inline-flex; } );
			...
			if ( static::WRAPPED_TAG ) :
				$value = '<span id="co-tag-' . esc_attr( $this->get_id() ) . '" class="co-tag">' . $value . '</span>';
			endif;
