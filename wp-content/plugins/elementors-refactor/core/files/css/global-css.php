...
/**
 * Drag & Drop Layout global CSS file.
 *
 * Drag & Drop Layout CSS file handler class is responsible for generating the global CSS
 * file.
 *
 * @since 1.2.0
 */
class Global_CSS extends Base {
	...
	/**
	 * Drag & Drop Layout global CSS file handler ID.
	 */
	const FILE_HANDLER_ID = 'co-global';
			...
			foreach ( $scheme_controls as $control ) {
				$this->add_control_rules(
					$control, $widget->get_controls(), function( $control ) use ( $elementor ) {
						...
						return $scheme_value;
					}, [ '{{WRAPPER}}' ], [ '.co-widget-' . $widget->get_name() ]
				);
			}
		}
	}
