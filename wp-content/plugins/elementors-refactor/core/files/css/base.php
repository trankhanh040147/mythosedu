...
/**
 * Drag & Drop Layout CSS file.
 *
 * Drag & Drop Layout CSS file handler class is responsible for generating CSS files.
 *
 * @since 1.2.0
 * @abstract
 */
abstract class Base extends Base_File {
	...
	/**
	 * Drag & Drop Layout CSS file generated status.
	 *
	...
	/**
	 * Drag & Drop Layout inline CSS status.
	 *
	...
	/**
	 * Drag & Drop Layout CSS empty status.
	 *
	 ...
	 *
	 * Initializing Drag & Drop Layout CSS file.
	 *
	 * @since 1.2.0
	 * @access public
	 */
	 ...
	 * Enqueue CSS.
	 *
	 * Either enqueue the CSS file in Drag & Drop Layout or add inline style.
	 *
	...
	protected function add_repeater_control_style_rules( Controls_Stack $controls_stack, array $repeater_control, array $repeater_values, array $placeholders, array $replacements ) {
		...
		foreach ( $repeater_control['style_fields'] as $index => $item ) {
			$this->add_controls_stack_style_rules(
				...
				$placeholders,
				array_merge( $replacements, [ '.co-repeater-item-' . $repeater_values[ $index ]['_id'] ] ),
	...
	protected function add_dynamic_control_style_rules( array $control, $value ) {
		Plugin::$instance->dynamic_tags->parse_tags_text( $value, $control, function( $id, $name, $settings ) {
			...
			$this->add_controls_stack_style_rules( $tag, $tag->get_style_controls(), $tag->get_active_settings(), [ '{{WRAPPER}}' ], [ '#co-tag-' . $id ] );
		} );
	}
}
