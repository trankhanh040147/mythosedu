...
abstract class Document extends Controls_Stack {
	...
	public function get_container_attributes() {
		...
		if ( version_compare( $version_meta, '2.5.0', '<' ) ) {
			$attributes['class'] .= ' co-bc-flex-widget';
		}

		if ( Plugin::$instance->preview->is_preview() ) {
			$attributes['data-co-title'] = static::get_title();
		} else {
			$attributes['data-co-settings'] = wp_json_encode( $this->get_frontend_settings() );
		}
	...
	public function get_autosave( $user_id = 0, $create = false ) {
		...
		if ( $autosave_id ) {
			...
			$autosave_id = wp_create_post_autosave( [
				...
				'post_content' => '<!-- Created With Drag & Drop Layout -->',
	...
	protected function _register_controls() {
		 ...
		 *
		 * Fires after Drag & Drop Layout registers the document controls.
		 *
		 * @since 2.0.0
		 *
		 * @param Document $this The document instance.
	...
	protected function save_elements( $elements ) {
		 ...
		 * Before saving data.
		 *
		 * Fires before Drag & Drop Layout saves data to the database.
		 *
		 * @since 1.0.0
		 *
		 ...
		 * After saving data.
		 *
		 * Fires after Drag & Drop Layout saves data to the database.
		 *
		 * @since 1.0.0
		 *
	...
	public function save_version() {
		if ( ! defined( 'IS_ELEMENTOR_UPGRADE' ) ) {
			 ...
			 * Will not fire during Drag & Drop Layout Upgrade.
			 *
			 * @since 2.5.12
			 *
			 * @param \Elementor\Core\Base\Document $this The current document.
