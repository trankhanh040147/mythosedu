 ...
 *
 * Drag & Drop Layout editor handler class is responsible for initializing Elementor
...
class Editor {
	...
	/**
	 * The nonce key for Drag & Drop Layout editor.
	 * @deprecated 2.3.0
	 */
	const EDITING_NONCE_KEY = 'co-editing';

	/**
	 * User capability required to access Drag & Drop Layout editor.
	 */
	 ...
	 * Init.
	 *
	 * Initialize Drag & Drop Layout editor. Registers all needed actions to run Elementor,
	 ...
	 * Redirect to new URL.
	 *
	 * Used as a fallback function for the old URL structure of Drag & Drop Layout page
	 * edit URL.
	 *
	...
	public function enqueue_scripts() {
		...
		wp_register_script(
			'ace',
            ELEMENTOR_ASSETS_URL . 'lib/extends/ace/ace.js',
			[],
			'1.2.5',
			true
		);
		...
		wp_register_script(
			'ace-language-tools',
            ELEMENTOR_ASSETS_URL . 'lib/extends/ace/ext-language_tools.js',
			[
				'ace',
			],
			'1.2.5',
		 ...
		 *
		 * Fires before Drag & Drop Layout editor scripts are enqueued.
		 *
		 * @since 1.0.0
		 */
		...
		$config = [
			...
			'i18n' => [
				'elementor' => __( 'Drag & Drop Layout', 'elementor' ),
				...
				'flexbox_attention_message' => __( 'Drag & Drop Layout 2.5 introduces key changes to the layout using CSS Flexbox. Your existing pages might have been affected, please review your page before publishing.', 'elementor' ),
				...
				// Menu.
				'about_elementor' => __( 'About Drag & Drop Layout', 'elementor' ),
				...
				'dialog_confirm_enable_svg' => __( 'Before you enable SVG upload, note that SVG files include a security risk. Drag & Drop Layout does run a process to remove possible malicious code, but there is still risk involved when using such files.', 'elementor' ),

				// Enable fontawesome 5 if needed.
				'enable_fa5' => __( 'Drag & Drop Layout New Icon Library', 'elementor' ),
				'dialog_confirm_enable_fa5' => __( 'Drag & Drop Layout v2.6 includes an upgrade from Font Awesome 4 to 5. In order to continue using icons, be sure to click "Upgrade".', 'elementor' ) . ' <a href="#" target="_blank">' . __( 'Learn More', 'elementor' ) . '</a>',
				...
				'preview_el_not_found_message' => __( 'You must call \'the_content\' function in the current template, in order for Drag & Drop Layout to work on this page.', 'elementor' ),
		...
		Utils::print_js_config( 'co-editor', 'ElementorConfig', $config );

		wp_enqueue_script( 'co-editor' );
		 ...
		 *
		 * Fires after Drag & Drop Layout editor scripts are enqueued.
		 *
		 * @since 1.0.0
		 */
	...
	public function enqueue_styles() {
		 ...
		 *
		 * Fires before Drag & Drop Layout editor styles are enqueued.
		 *
		 * @since 1.0.0
		 */
		...
		if ( Responsive::has_custom_breakpoints() ) {
			...
			wp_add_inline_style( 'co-editor', '.co-device-tablet #co-preview-responsive-wrapper { width: ' . $breakpoints['md'] . 'px; }' );
		}

		/**
		 * After editor enqueue styles.
		 *
		 * Fires after Drag & Drop Layout editor styles are enqueued.
		 *
		 * @since 1.0.0
		 */
	 ...
	 *
	 * Config the default WordPress editor with custom settings for Drag & Drop Layout use.
	 *
	 * @since 1.9.0
	 * @access private
	 */
	...
	public function editor_head_trigger() {
		/**
		 * Drag & Drop Layout editor head.
		 *
		 * Fires on Drag & Drop Layout editor head tag.
		 *
	 ...
	 * WP footer.
	 *
	 * Prints Drag & Drop Layout editor with all the editor templates, and render controls,
	...
	public function wp_footer() {
		...
		/**
		 * Drag & Drop Layout editor footer.
		 *
		 * Fires on Drag & Drop Layout editor before closing the body tag.
		 *
	 ...
	 * Editor constructor.
	 *
	 * Initializing Drag & Drop Layout editor and redirect from old URL structure of
	 * Drag & Drop Layout editor.
	 *
	 * @since 1.0.0
	 * @access public
	 ...
	 * give him access to Drag & Drop Layout editor.
	 *
	 * @since 1.8.1
