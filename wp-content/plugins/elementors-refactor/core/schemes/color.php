...
/**
 * Drag & Drop Layout color scheme.
 *
 * Drag & Drop Layout color scheme class is responsible for initializing a scheme for
 * colors.
 *
 * @since 1.0.0
 */
class Color extends Base_UI {
	...
	public function print_template_content() {
		?>
		<div class="co-panel-scheme-content co-panel-box">
			<div class="co-panel-heading">
				<div class="co-panel-heading-title"><?php echo $this->_get_current_scheme_title(); ?></div>
			</div>
			<?php
			...
			if ( $description ) :
				?>
				<div class="co-panel-scheme-description co-descriptor"><?php echo $description; ?></div>
			<?php endif; ?>
			<div class="co-panel-scheme-items co-panel-box-content"></div>
		</div>
		<div class="co-panel-scheme-colors-more-palettes co-panel-box">
			<div class="co-panel-heading">
				<div class="co-panel-heading-title"><?php echo __( 'More Palettes', 'elementor' ); ?></div>
			</div>
			<div class="co-panel-box-content">
				<?php foreach ( $this->_get_system_schemes_to_print() as $scheme_name => $scheme ) : ?>
					<div class="co-panel-scheme-color-system-scheme" data-scheme-name="<?php echo esc_attr( $scheme_name ); ?>">
						<div class="co-panel-scheme-color-system-items">
							<?php foreach ( $scheme['items'] as $color_value ) : ?>
								<div class="co-panel-scheme-color-system-item" style="background-color: <?php echo esc_attr( $color_value ); ?>;"></div>
							<?php endforeach; ?>
						</div>
						<div class="co-title"><?php echo $scheme['title']; ?></div>
					</div>
				<?php endforeach; ?>
			</div>
		</div>
