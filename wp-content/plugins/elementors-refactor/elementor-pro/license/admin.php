...
class Admin {
	...
	public function action_activate_license() {
		...
		if ( empty( $_POST['elementor_pro_license_key'] ) ) {
			wp_die( __( 'Please enter your license key.', 'elementor-pro' ), __( 'Drag & Drop Layout Pro', 'elementor-pro' ), [
				'back_link' => true,
			] );
		}
		...
		if ( is_wp_error( $data ) ) {
			wp_die( sprintf( '%s (%s) ', $data->get_error_message(), $data->get_error_code() ), __( 'Drag & Drop Layout Pro', 'elementor-pro' ), [
				'back_link' => true,
			] );
		}
		...
		if ( API::STATUS_VALID !== $data['license'] ) {
			...
			wp_die( $error_msg, __( 'Drag & Drop Layout Pro', 'elementor-pro' ), [
				'back_link' => true,
			] );
		}
		...
		<div class="wrap elementor-admin-page-license">
			...
			<form class="elementor-license-box" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<?php else :
					...
					<?php if ( API::STATUS_EXPIRED === $license_data['license'] ) : ?>
					<p class="e-row-divider-bottom elementor-admin-alert elementor-alert-danger"><?php printf( __( '<strong>Your License Has Expired.</strong> <a href="%s" target="_blank">Renew your license today</a> to keep getting feature updates, premium support and unlimited access to the template library.', 'elementor-pro' ), '#' ); ?></p>
				<?php endif; ?>
		...
		if ( empty( $license_key ) ) {
			...
			<div class="notice elementor-message">
				<div class="elementor-message-inner">
					...
					<div class="elementor-message-content">
						<strong><?php echo __( 'Welcome to Drag & Drop Layout Pro!', 'elementor-pro' ); ?></strong>
						<p><?php echo $this->get_activate_message(); ?></p>
					</div>
	...
	private function handle_dashboard_admin_widget() {
		add_action( 'elementor/admin/dashboard_overview_widget/after_version', function() {
			/* translators: %s: Drag & Drop Layout Pro version. */
			echo '<span class="e-overview__version">' . sprintf( __( 'Drag & Drop Layout Pro v%s', 'elementor-pro' ), ELEMENTOR_PRO_VERSION ) . '</span>';
		} );
		...
		<div class="wrap elementor-admin-page-license">
			...
			<form class="elementor-license-box" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<?php if ( empty( $license_key ) ) : ?>
					...
					<p><?php _e( 'Enter your license key here, to activate Drag & Drop Layout Pro, and get feature updates, premium support and unlimited access to the template library.', 'elementor-pro' ); ?></p>

					<ol>
						<li><?php printf( __( 'Log in to <a href="%s" target="_blank">your account</a> to get your license key.', 'elementor-pro' ), '#' ); ?></li>
						<li><?php printf( __( 'If you don\'t yet have a license key, <a href="%s" target="_blank">get Drag & Drop Layout Pro now</a>.', 'elementor-pro' ), '#' ); ?></li>
				...
				<?php else :
					...
					<?php if ( API::STATUS_EXPIRED === $license_data['license'] ) : ?>
					<p class="elementor-admin-alert elementor-alert-danger"><?php printf( __( '<strong>Your License Has Expired.</strong> <a href="%s" target="_blank">Renew your license today</a> to keep getting feature updates, premium support and unlimited access to the template library.', 'elementor-pro' ), '#' ); ?></p>
				<?php endif; ?>
