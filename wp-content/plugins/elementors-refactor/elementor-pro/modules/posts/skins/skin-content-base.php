...
trait Skin_Content_Base {
	...
	public function render_post_content( $with_wrapper = false ) {
		...
		if ( Plugin::elementor()->preview->is_preview_mode( $post->ID ) ) {
			...
			// Print manually (and don't use `the_content()`) because it's within another `the_content` filter, and the Drag & Drop Layout filter has been removed to avoid recursion.
