...
use Elementor\Tools;
use Elementor\Utils;
use Elementor\Core\Admin\Component AS CPT;
...
class Admin extends App {
	...
	public function plugin_row_meta( $plugin_meta, $plugin_file ) {
		if ( ELEMENTOR_PRO_PLUGIN_BASE === $plugin_file ) {
			$plugin_slug = basename( ELEMENTOR_PRO__FILE__, '.php' );
			$plugin_name = __( 'Drag & Drop Layout Pro', 'elementor-pro' );
			...
			$row_meta = [
				'view-details' => sprintf( '<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>',
					...
					/* translators: %s: Plugin name - Drag & Drop Layout Pro. */
					esc_attr( sprintf( __( 'More information about %s', 'elementor-pro' ), $plugin_name ) ),
					esc_attr( $plugin_name ),
					__( 'View details', 'elementor-pro' )
				),
				'changelog' => '<a href="#" title="' . esc_attr( __( 'View Drag & Drop Layout Pro Changelog', 'elementor-pro' ) ) . '" target="_blank">' . __( 'Changelog', 'elementor-pro' ) . '</a>',
			];
