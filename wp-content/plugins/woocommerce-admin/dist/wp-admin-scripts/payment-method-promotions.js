this["wc"] = this["wc"] || {}; this["wc"]["paymentMethodPromotions"] =
...
const PaymentPromotionRow = _ref => {
  ...
  const {
    ...
    if (isActive) {
      paymentGatewayData = getPaymentGateway(pluginSlug.replace(/\-/g, '_'));
    }
  ...
  const installPaymentGateway = () => {
    ...
    setInstalling(true);
    Object(external_wc_tracks_["recordEvent"])('settings_payments_recommendations_setup', {
      extension_selected: pluginSlug
    });
    installAndActivatePlugins([pluginSlug]).catch(response => {
      if (response.message) {
