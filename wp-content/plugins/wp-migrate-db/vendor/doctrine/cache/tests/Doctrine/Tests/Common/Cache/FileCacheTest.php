 ...
 * @group DCOM-101
 */
class FileCacheTest extends \DeliciousBrains\WPMDB\Container\Doctrine\Tests\DoctrineTestCase
{
    /**
    ...
    {
        $this->driver = $this->getMock('DeliciousBrains\\WPMDB\\Container\\Doctrine\\Common\\Cache\\FileCache', array('doFetch', 'doContains', 'doSave'), array(), '', \false);
    }
    public function getProviderFileName()
    {
        return array(
            ...
            array('key:1', 'key-1'),
            array('DeliciousBrains\\WPMDB\\Container\\key\\2', 'key-2'),
            array('key/3', 'key-3'),
            array('key<4', 'key-4'),
            array('key>5', 'key-5'),
            array('key"6', 'key-6'),
    ...
    {
        $data = array('key:0' => 'key-0', 'DeliciousBrains\\WPMDB\\Container\\key' => 'key-0', 'key/0' => 'key-0', 'key<0' => 'key-0', 'key>0' => 'key-0', 'key"0' => 'key-0', 'key*0' => 'key-0', 'key?0' => 'key-0', 'key|0' => 'key-0', 'key-0' => 'key__0', 'keyä0' => 'key--0');
        $paths = array();
        $cache = $this->driver;
        $method = new \ReflectionMethod($cache, 'getFilename');
        $method->setAccessible(\true);
    ...
    public function testFilenameShouldCreateThePathWithFourSubDirectories()
    {
        ...
        $method = new \ReflectionMethod($cache, 'getFilename');
        $key = 'item-key';
        $expectedDir = array('84', 'e0', 'e2', 'e8', '93', 'fe', 'bb', '73', '7a', '0f', 'ee', '0c', '89', 'd5', '3f', '4b', 'b7', 'fc', 'b4', '4c', '57', 'cd', 'f3', 'd3', '2c', 'e7', '36', '3f', '5d', '59', '77', '60');
    ...
    {
        $driver1 = $this->getMock('DeliciousBrains\\WPMDB\\Container\\Doctrine\\Common\\Cache\\FileCache', array('doFetch', 'doContains', 'doSave'), array(__DIR__, '.*'));
        $driver2 = $this->getMock('DeliciousBrains\\WPMDB\\Container\\Doctrine\\Common\\Cache\\FileCache', array('doFetch', 'doContains', 'doSave'), array(__DIR__, '.php'));
    ...
    {
        $driver = $this->getMock('DeliciousBrains\\WPMDB\\Container\\Doctrine\\Common\\Cache\\FileCache', array('doFetch', 'doContains', 'doSave'), array(__DIR__ . '/../', '/' . \basename(__FILE__)));
