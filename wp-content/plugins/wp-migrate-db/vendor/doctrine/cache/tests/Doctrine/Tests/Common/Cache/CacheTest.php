...
use ArrayObject;
abstract class CacheTest extends \DeliciousBrains\WPMDB\Container\Doctrine\Tests\DoctrineTestCase
{
    /**
    ...
    public function testFetchMultiWillFilterNonRequestedKeys()
    {
        ...
        $cache = $this->getMockForAbstractClass('DeliciousBrains\\WPMDB\\Container\\Doctrine\\Common\\Cache\\CacheProvider', array(), '', \true, \true, \true, array('doFetchMultiple'));
        $cache->expects($this->once())->method('doFetchMultiple')->will($this->returnValue(array('[foo][]' => 'bar', '[bar][]' => 'baz', '[baz][]' => 'tab')));
        $this->assertEquals(array('foo' => 'bar', 'bar' => 'baz'), $cache->fetchMultiple(array('foo', 'bar')));
    }
