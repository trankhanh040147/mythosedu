...
class ContentDrip {
	...
	public function tutor_content_drip_state_update() {
		...
		$course_id = sanitize_text_field( tutor_utils()->array_get('course_id', $_POST) );
			...
			'fields'     => array(
				...
				'_tutor_course_settings[content_drip_type]'   => array(
					...
					'options' => array(
						...
						'specific_days'                 => __( 'Content available after X days from enrolment', 'tutor-pro' ),
	...
	public function is_lock_lesson( $post = null ) {
		...
		if ( $drip_type === 'unlock_by_date' ) {
			...
			if ( $days > 0 ) {
				$enroll       = tutor_utils()->is_course_enrolled_by_lesson( $lesson_id );
	...
	public function register_emails( $attr ) {
		$attr['email_to_students']['new_lesson_published'] = array(
			...
			'subject'          => __( 'A New Lesson Has Been Published', 'tutor-pro' ),
			'heading'          => __( 'New Lesson Published', 'tutor-pro' ),
			'message'          => __( 'A new lesson has been added for the course <strong>{course_name}</strong>.', 'tutor-pro' ),
		...
		$attr['email_to_students']['new_quiz_published'] = array(
			...
			'subject'          => __( 'A New Quiz Has Been Published', 'tutor-pro' ),
		...
		$attr['email_to_students']['new_assignment_published'] = array(
			...
			'subject'         => __( 'A New Assignment Has Been Published', 'tutor-pro' ),
			'student_name'    => __( 'Student', 'tutor-pro' ),
	...
	// Get enrolment list of published course by course ID
