	...
	public static function tt4b_admin_menu_main() {
		...
		if ( false !== $access_token ) {
			...
			if ( [] === $business_profile['data'] ) {
				...
				if ( array_key_exists( 'catalog_id', $business_profile['data'] ) && ! is_null( $business_profile['data']['catalog_id'] ) ) {
					$catalog_id = $business_profile['data']['catalog_id'];
					update_option( 'tt4b_catalog_id', $catalog_id );
				}

				if ( array_key_exists( 'catalog_id', $business_profile['data'] ) && ! is_null( $business_profile['data']['catalog_id'] ) && ! is_null( $business_profile['data']['bc_id'] ) ) {
					if ( did_action( 'woocommerce_loaded' ) > 0 ) {
