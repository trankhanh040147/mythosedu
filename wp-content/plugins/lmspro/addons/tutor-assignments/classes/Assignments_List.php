...
class Assignments_List {
	...
	public function column_student( $item ) {
		echo '<img src="' . get_avatar_url( $item->user_id, array( 'size' => 20 ) ) . '"/>';
		echo '<div class="tutor-fs-6 tutor-fw-medium tutor-color-black">' . $item->comment_author . '</div>';
	}
	...
	public function column_title( $item ) {
		...
		echo '<a href="' . get_the_permalink( $item->comment_post_ID ) . '">' . get_the_title( $item->comment_post_ID ) . '</a>';
		echo '<div class="course-meta">
				<span class="tutor-color-secondary tutor-fs-7">
					<strong class="tutor-fs-7 tutor-fw-medium">' . __( 'Course', 'tutor-pro' ) . ': </strong>' . $item->post_title .
				'</span>
			 </div>';
	}
	...
	public function column_action_evaluate( $item, $post_id ) {
		...
		echo "<a class='tutor-btn tutor-btn-outline-primary tutor-btn-sm' href='" . admin_url( "admin.php?page=tutor-assignments&view_assignment=$item->comment_ID&post-id=$post_id" ) . "'>" . $button_text . '</a>';
	}
	...
	public function column_action_delete( $item ) {
		echo '<a class="tutor-btn tutor-btn-outline-primary tutor-btn-sm" data-assignment_id="' . esc_attr( $item->comment_ID ) . '" href="#" data-tutor-modal-target="assignment-' . esc_attr( $item->comment_ID ) . '">' . _x( ' Delete', 'assignment delete', 'tutor-pro' ) . '</a>';
	}
	...
	public static function assignment_list_all( string $status, $course_id = '', $date = '', $search_term = '', $offset = '', $limit = '', $order = '', $user_id = 0 ) {
		return $results = self::count_all( $status, $course_id, $date, $search_term, $offset, $limit, $order, $user_id );
	}
	...
	public static function assignment_list_pending( string $status, $course_id = '', $date = '', $search_term = '', $offset = '', $limit = '', $order = '', $user_id = 0 ) {
		return $results = self::count_pending( $status, $course_id, $date, $search_term, $offset, $limit, $order, $user_id );
	}
	...
	public static function assignment_list_pass_fail( string $status, $course_id = '', $date = '', $search_term = '', $offset = '', $limit = '', $order = '', $user_id = 0 ) {
		return $results = self::count_pass_fail( $status, $course_id, $date, $search_term, $offset, $limit, $order, $user_id );
	}
	...
	protected static function count_pass_fail( string $status, $course_id = '', $date = '', $search_term = '', $offset = '', $limit = '', $order = '', $user_id = 0 ) {
		...
		$status_query = '';
		if ( 'pass' === $status ) {
			$status_query = 'AND evaluate_mark.meta_value >= pass_mark.meta_value';
		} else {
			$status_query = 'AND evaluate_mark.meta_value < pass_mark.meta_value';
		}
