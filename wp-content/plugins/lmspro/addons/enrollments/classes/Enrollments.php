...
class Enrollments {
	...
	public function enrol_student() {
		...
		if(!$user_id){
			$this->success_msgs = get_tnotice( __( 'Please select student', 'tutor-pro' ), 'Error', 'danger' );
			add_filter( 'student_enrolled_to_course_msg', array( $this, 'return_message' ) );
			return;
		}		
		if(!$user_id){
			$this->success_msgs = get_tnotice( __( 'Please select course', 'tutor-pro' ), 'Error', 'danger' );
			add_filter( 'student_enrolled_to_course_msg', array( $this, 'return_message' ) );
			return;
		}	
		...
		if ( $is_enrolled ) {
			...
			if ( $is_enrolled ) {
				...
				$this->success_msgs = get_tnotice( __( 'Enrolment failed! Sorry!', 'tutor-pro' ), 'Error', 'danger' );
