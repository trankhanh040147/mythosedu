...
class Analytics extends ExportAnalytics {
	...
	/**
	 * Get total enrolment / students by user_id, optionally can set period ( today | monthly| yearly )
	 *
	 * Optionally can set start date & end date to get enrolment list from date range
	 *
	 * If period or date range not pass then it will return all time enrolment list
	 *
	 * @param $user_id int | required
	 *
	 * @param $period string | optional
	 ...
	 *
	 * It will return data as per query vars for Earnings | Enrolment list | Refunds | Discounts
	 *
	 * @return array
	 *
	 * @since 1.9.9
	...
	public static function get_last_enrolled_courses( int $limit = 5 ) {
		...
		return $wpdb->get_results( $wpdb->prepare( $sql, 'tutor_enrolled', 'completed', 'courses', $limit ) );
	}
