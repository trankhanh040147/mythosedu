	...
	public function is_lock_lesson( $content_id ) {
		...
		if ( $drip_type === 'unlock_by_date' ) {
			...
			if ( $days > 0 ) {
				...
				$unlock_date          = date_i18n( get_option( 'date_format' ), $unlock_timestamp );
				$this->unlock_message = sprintf( __( 'This %s will be available for you from %s', 'tutor-pro' ), $this->singular_post_type, $unlock_date );
	...
	public function drip_content_protection( $html ) {
		...
		if ( $this->is_lock_lesson( $content_id ) ) {
			...
			if(get_post_field( 'post_type', $content_id )!='tutor_quiz') {
				ob_start();
				tutor_load_template( 'single.common.header', array( 'course_id' => $course_id, 'mark_as_complete' => false ) );
				$header = ob_get_clean();
			}
			...
			} else if($this->drip_type=='unlock_sequentially' && $previous_id) {
