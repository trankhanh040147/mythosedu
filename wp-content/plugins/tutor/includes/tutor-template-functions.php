...
if ( ! function_exists( 'tutor_get_template' ) ) {
	function tutor_get_template( $template = null, $tutor_pro = false ) {
		...
		if ( ! file_exists( $template_location ) ) {
			...
			if ( ! file_exists( $template_location ) ) {
				echo '<div class="tutor-notice-warning"> ' . __( sprintf( 'The file you are trying to load does not exist in your theme or LMS plugin location. If you are extending the LMS plugin, please create a php file here: %s ', '<code>' . $file_in_theme . '</code>' ), 'tutor' ) . ' </div>';
			}
		}
