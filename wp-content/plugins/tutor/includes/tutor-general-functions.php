...
//HuyNote : edit this function July31, 22
if ( ! function_exists( 'tutor_course_list_dropdown' ) ) {
	function tutor_course_list_dropdown( $post_ID = 0, $args = array() ) {

		$default = array(
			'classes'  => '',
			'name'     => '_tutor_course_relative',
			'multiple' => false,	// HuyNote: change from true to false (select one only)
		);

		$args = apply_filters( 'tutor_courses_list_dropdown_args', array_merge( $default, $args ) );
		...
		$parent_id = wp_get_post_parent_id($post_ID); //get parent_course
		if(!$parent_id){//if no parent course, get first child course
			$child_courses = new WP_Query( array(
											'post_parent' => $post_ID,
											'posts_per_page' => -1,
											'post_type' => 'courses', 
										) 
									);
			if(count($child_courses->posts)>0) {
				$child_id =  $child_courses->posts[0]->ID;
			}
		}
		$output  = '';
		$output .= '<div class="row">
						<div class="col-md-4 pb-2">
							<span for="parent_course"><input type="radio" id="parent_course" name="course_relative" value="parent_course" ';
		if(!$child_id) $output .= ' checked = "checked" '; 
		$output .= ' > Choose a Parent course</span>
						</div>
						<div class="col-md-4 pb-2">
							<span for="child_course"><input type="radio" id="child_course" name="course_relative" value="child_course" ';
		if(!$parent_id && $child_id) $output .= ' checked = "checked" '; 
		$output .= ' > Choose a Child course</span>
						</div>
					</div>
					<div class="tutor-form-field-course-list">';			
					
		$output .= '<select name="' . $name . '" ' . $multiple_select . ' class="' . $classes . '" data-placeholder="' . __( 'Search Relative Course. ex. Course 1 or Course 2,..', 'tutor' ) . '">';
		$output .= '<option value="">' . __( 'Select a course', 'tutor' ) . '</option>';
		
		foreach($courses_list as $citem) {
			//echo $citem->ID."--";
			if($citem->ID==$post_ID) continue;	// HuyNote: do not get current course
			$output .= '<option ';
			if($parent_id == $citem->ID)  $output .= ' selected="true" ';
			if($child_id == $citem->ID)  $output .= ' selected="true" ';
			$output .= ' value="'. $citem->ID.'">' . $citem->post_title . '</option>';
		}
		//$output .= _generate_categories_dropdown_option( $post_ID, $courses_list, $args );
		$output .= '</select></div>';
