...
class Template extends Tutor_Base {
	...
	public function load_single_course_template( $template ) {
		...
		if ( $wp_query->is_single && ! empty( $wp_query->query_vars['post_type'] ) && $wp_query->query_vars['post_type'] === $this->course_post_type ) {
			...
			$parent_ids = get_post_meta( get_the_ID(), '_tutor_course_parent', true );
			$parent_ids_arr = array();
			if($parent_ids)
				$parent_ids_arr = explode(" ",trim($parent_ids));
			//var_dump($parent_ids_arr);
			if ( !count($parent_ids_arr) && count($children_ids_arr)) {// if this course is parent course
				return tutor_get_template( 'single-course-parent' );
			}
			else{//return tutor_get_template( 'single-course' );
				// (if has parent) - this course is child course
				if ( is_user_logged_in()) // if logged in
					return tutor_get_template( 'single-course-child-logged' );
				else //if has not logged in 
					return tutor_get_template( 'single-course-child-not-logged' );	
