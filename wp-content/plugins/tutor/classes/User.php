...
class User {
	...
	public function profile_update($user_id){
		...
		$ip_DepartmentName        	  = wp_kses_post( tutor_utils()->avalue_dot( 'ip_DepartmentName', $_POST ) );
		//$ip_DepartmentName		  	  =	str_replace(' ', '', $ip_DepartmentName);
		$ip_DepartmentName		  	  =	implode(" , ", $ip_DepartmentName); //var_dump($ip_DepartmentName); die();
		...
		$wpdb->query( $wpdb->prepare("UPDATE {$wpdb->users} 
											...
											WHERE ID = %d",$ip_DepartmentName,$user_id)
								);
		$wpdb->query( $wpdb->prepare("UPDATE {$wpdb->users} 
