...
class Course extends Tutor_Base {
	...
	public function __construct() {
		...
		//add_action('woocommerce_thankyou', 'enroll_student_course', 10, 1);
	}
	
	//NhatHuy
	function enroll_student_course( $order_id ) {
		if ( ! $order_id )
			return;
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$course_id = (int)tutor_utils()->get_product_course_id($product_id); 
			if($course_id){
	...
	public function save_course_meta( $post_ID, $post ) {
		...
		/** NhatHuy - NoteBegin */
		/** NhatHuy - Save Course type */
		if ( ! empty($_POST['_tutor_course_type'])){
			$course_type = array();
			$course_type['course_type'] = $_POST['_tutor_course_type'];
			if(isset($_POST['internal_pay_ot_not']) && $_POST['internal_pay_ot_not'])
				$course_type['internal_pay_ot_not'] = $_POST['internal_pay_ot_not'];	
			$course_type = maybe_serialize($course_type);
			update_post_meta($post_ID, '_tutor_course_type', $course_type);
		}
		
		/** NhatHuy - NoteEnd */
		
		/** NhatHuy - Save Course Code */
		$course_code = sanitize_text_field($_POST['_tutor_course_code_start']).sanitize_text_field($_POST['_tutor_course_code_end']);
		...
		//Ky
		//StartTime & EndTime
		if ( ! empty($_POST['_tutor_course_start_time'])){
			$tutor_course_start_time = sanitize_text_field($_POST['_tutor_course_start_time']); //var_dump($tutor_course_start_time); die();
			update_post_meta($post_ID, '_tutor_course_start_time', $tutor_course_start_time);
		}
		if ( ! empty($_POST['_tutor_course_end_time'])){
			$tutor_course_end_time = sanitize_text_field($_POST['_tutor_course_end_time']);
			update_post_meta($post_ID, '_tutor_course_end_time', $tutor_course_end_time);
		}
		/**
...
//die();
	...
	public function unenroll_now() {
		...
		if (is_array($children_ids_arr) && count($children_ids_arr) ){
			tutor_utils()->do_uncompleted( $course_id , $user_id);
...
//NHatHuy: parent course - add to cart action - check if add children?
	public function add_children_now() {
		global $woocommerce;
		// Checking if action comes from Enroll form
		if ( tutor_utils()->array_get( 'tutor_course_action', tutor_sanitize_data($_POST) ) !== '_tutor_add_children' || ! isset( $_POST['tutor_course_id'] ) ) {
			return;
		}
		...
		//}
		//else {
			tutor_utils()->do_enroll( $course_id );//die();
		//}
		
		$add_children = $_POST['add-children'];
		if($add_children=="add-children"){	//add children checked				
			...
			if(count($children_ids_arr)){
				foreach($children_ids_arr as $cid){
					...
					if ( $is_purchasable_child ) {
						$product_id_child = tutor_utils()->get_course_product_id($cid);
						$woocommerce->cart->add_to_cart($product_id_child, 1);
					}
				}
			}
			$referer_url = '/cart';
