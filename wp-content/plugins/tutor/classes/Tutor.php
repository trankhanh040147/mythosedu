...
final class Tutor {
	...
	public static function tutor_activate() {		
		...
		$column_DepartmentName  = "DepartmentName";
		...
		$result_column_DepartmentName =  $wpdb->get_results("SHOW COLUMNS FROM `".$table_users."` LIKE '".$column_DepartmentName."'");
		if(empty($result_column_DepartmentName))
			$wpdb->query( $wpdb->prepare("ALTER TABLE `".$table_users."` ADD `".$column_DepartmentName."` VARCHAR(100) default '';"));
