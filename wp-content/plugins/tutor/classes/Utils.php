 ...
 * @Author LLH llh all functions lms
 * @package TUTOR
 *
 * @since v.1.0.0
 */
...
class Utils {
	...
	public function get_DepartmentName($user_id) {
		global $wpdb;
		$entry = $wpdb->get_row( "SELECT DepartmentName FROM {$wpdb->users} 
									WHERE ID = {$user_id}");
		if (isset($entry)){					
	...
	public function get_DepartmentName_name($DepartmentName_profile) {
		global $wpdb;

		if(!$DepartmentName_profile) return "";
		$manage_branchs_arr = explode(",",$DepartmentName_profile);
		$manage_branchs_arr_new = array();
		$res=array();
		if(count($manage_branchs_arr)){
			foreach($manage_branchs_arr as $br){
				if (array_key_exists($br, _DEFINE_GROUPS_USER)) {
					$res[]=_DEFINE_GROUPS_USER[$br];
				}
				$manage_branchs_arr_new[] = "'".$br."'";
			}
			$manage_branchs_new = implode(',',$manage_branchs_arr_new);
		...
		$query = $wpdb->get_results(
			...
			FROM 	{$wpdb->users}
			WHERE DepartmentName IN ({$manage_branchs_new})
			"
			)
		);
		...
		if(isset($query)){
			foreach($query as $q){
				if(!in_array($q->DepartmentName,$res))
					$res[]=$q->DepartmentName;
			}
		}
	...
	public function get_user_manage_students_all($user_id, $offset=0, $per_page=1000, $user_group="") {
		global $wpdb;
	...
	//NhatHuy
	public function get_courses_for_students( $limit = 6 ) {
		global $wpdb;

		$course_post_type = tutor()->course_post_type;
		$exclude_courses = $this->get_enrolled_courses_ids_by_user();
		$courses = get_posts(
			array(
				'post_type'      => $course_post_type,
				'meta_query' =>  [
	...
	//NhatHuy
	public function get_courses_children( $course_id ) {
		global $wpdb;
		...
		$children_ids_str = implode(",",$children_ids_arr);							
		$courses = get_posts(
			array(
				'post_type'      => $course_post_type,
				'post_status'    => array( 'publish' ),
				'post__in'    => $children_ids_arr,
				'orderby' => 'post__in',				
				'posts_per_page'   => -1,	
			)
		);
	...
	//NhatHuy
	public function get_courses_frequently( $exclude_course = 0, $limit = 4 ) {
		global $wpdb;
	...
	public function checking_nonce( $request_method = null ) {
		...
		if ( ! $matched ) {
			wp_send_json_error( array( 'message' => __( 'Nonce not matched. Action failed!', TPL_DOMAIN_LANG) ) );
			exit;
		}
	}
...
//NhatHuy	
	public function get_attachments_by_course( $post_id = 0, $meta_key = '_tutor_attachments' ) {
		$post_id         = $this->get_post_id( $post_id );
		$lessons = $this->get_course_content_ids_by( tutor()->lesson_post_type, tutor()->course_post_type, $post_id );
		$attachments_arr = array();
		foreach($lessons as $lesson_id){
			$attachments     = maybe_unserialize( get_post_meta( $lesson_id, $meta_key, true ) );
			if ( is_array( $attachments ) && count( $attachments ) ) {
				foreach ( $attachments as $attachment ) {
					$data              = (array) $this->get_attachment_data( $attachment );
					$attachments_arr[] = (object) apply_filters( 'tutor/posts/attachments', $data );
...
//NhatHuy: updated Aug02, 2022
public function wpse_huy_apply_sort_course( $active_courses=array(), $posts ) {
    
	$sort_children_posts      = array();
	foreach( $posts as $post ) {
		...
		foreach($children_ids_arr as $cid){
			...
			if(in_array($cid, $active_courses)){
				...
				$post_child->subchild = '_'.$ii.'_';
				$sort_children_posts[$cid] = $post_child; 
			}
		}
		if (!array_key_exists($post->ID, $sort_children_posts))
			$sort_children_posts[$post->ID] = $post; 
	}
	
	
	$sort_ancestors_posts      = array();
	foreach( $sort_children_posts as $post ) {
        $ancestors_res = get_post_ancestors( $post->ID );
		if(count($ancestors_res)){
			$anc = array_reverse( $ancestors_res );
			$post->rowadd = $anc[0];
			$output = "";
			foreach ( $anc as $ancestor ) {
				$output .= $ancestor.'--';
			}
			$key = $output.$post->subchild.$post->ID;
		}
		else{
			$key = $post->ID.'---'; 
			$post->rowadd = $post->ID;
		}
		$sort_ancestors_posts[$key] = $post; 
    }
	ksort( $sort_ancestors_posts );
	//var_dump($sort_ancestors_posts);
    
	return $sort_ancestors_posts;
}
	/**
	 * @param int $user_id
	 *
...
//NhatHuy: updated Aug03, 2022
	 ...
	 * @since v.1.0.0
	 */
	public function get_active_courses_by_user_reorder( $user_id = 0, $offset = 0, $posts_per_page = -1 ) {
		$user_id             = $this->get_user_id( $user_id );
		$course_ids          = $this->get_completed_courses_ids_by_user( $user_id );
		$enrolled_course_ids = $this->get_enrolled_courses_ids_by_user( $user_id );
		$active_courses      = array_diff( $enrolled_course_ids, $course_ids );
		...
		if ( count( $active_courses ) ) {
			...
			$normal_query = new \WP_Query( $course_args );
			$filtered_posts = $this->wpse_huy_apply_sort_course( $active_courses, $normal_query->posts );
...
//NhatHuy: updated Sep07, 2022
	 ...
	 * @since v.1.0.0
	 */
	public function get_active_courses_by_user_learning( $user_id = 0, $offset = 0, $posts_per_page = -1 ) {
		$user_id             = $this->get_user_id( $user_id );
		$course_ids          = $this->get_completed_courses_ids_by_user( $user_id );
		$enrolled_course_ids = $this->get_enrolled_courses_ids_by_user( $user_id );
		$active_courses      = array_diff( $enrolled_course_ids, $course_ids );
	...
	public function do_enroll( $course_id = 0, $order_id = 0, $user_id = 0 ) {
		...
		$user_id = $this->get_user_id( $user_id );
		$title   = __( 'Course Enrolled', TPL_DOMAIN_LANG) . ' &ndash; ' . date( get_option( 'date_format' ) ) . ' @ ' . date( get_option( 'time_format' ) );
		...
		//NhatHuy:	enroll children course
			...
			if(count($children_ids_arr)){
				foreach($children_ids_arr as $cid){
					...
					//just do ernroll free children courses
	...
	public function get_product_course_id( $product_id = 0 ) {
		$course_id  = 0;
		if($product_id){
			$query =  new WP_Query( array( 'post_type' => 'courses', 'meta_key' => '_tutor_course_product_id', 'meta_value' => $product_id ) );
	...
	public function tutor_dashboard_pages() {
		...
		$new_navs      = apply_filters(
			...
			array(
				...
				'settings'    => array(
					'title' => __( 'Settings', TPL_DOMAIN_LANG),
					'icon'  => 'tutor-icon-settings-filled',
				),
				'logout'      => array(
					'title' => __( 'Logout', TPL_DOMAIN_LANG),
		...
		$dashboard_permalinks = apply_filters(
			...
			array(
				'retrieve-password' => array(
					'title'         => __( 'Retrieve Password', TPL_DOMAIN_LANG),
					'login_require' => false,
				),
			)
		);
...
//NhatHuy	
	public function tutor_dashboard_student_nav_ui_items() {
		$nav_items      = apply_filters(
			'tutor_dashboard/student_nav_ui_items',
			array(
				'index'            => array(
					'title' => __( 'Dashboard', TPL_DOMAIN_LANG),
					'icon'  => 'tutor-icon-dashboard-filled',
				),				
				'learning-courses'         => array(
					'title' => __( 'Learning Courses', TPL_DOMAIN_LANG),
					'icon'  => 'tutor-icon-book-open-filled',
				),
				'completed-courses'         => array(
					'title' => __( 'Completed Courses', TPL_DOMAIN_LANG),
		...
		return apply_filters( 'tutor_dashboard/student_nav_ui_items', $nav_items );
	}
	/**
	...
	public function instructor_status( $user_id = 0, $status_name = true ) {
		...
		$instructor_status = apply_filters(
			...
			array(
				'pending'  => __( 'Pending', TPL_DOMAIN_LANG),
				'approved' => __( 'Approved', TPL_DOMAIN_LANG),
				'blocked'  => __( 'Blocked', TPL_DOMAIN_LANG),
			)
		);
		...
		<div class="tutor-ratings <?php echo $css_class; ?>">
			...
			if ( $show_avg_rate ) {
				...
				<span class="tutor-rating-text tutor-fs-6 tutor-fw-normal tutor-color-black-60 tutor-pl-0 tutor-ml-0">
					...
					echo $current_rating;
					if ( ! ( $total_count === null ) ) {
						echo '&nbsp;(' . $total_count . ' ' . ( $total_count > 1 ? __( 'Ratings', TPL_DOMAIN_LANG) : __( 'Rating', TPL_DOMAIN_LANG) ) . ')';
					}
					?>
				</span>
				<?php
	...
	public function course_levels( $level = null ) {
		$levels = apply_filters(
			'tutor_course_level',
			array(
				'all_levels'   => __( 'All Levels', TPL_DOMAIN_LANG),
				'beginner'     => __( 'Beginner', TPL_DOMAIN_LANG),
				'intermediate' => __( 'Intermediate', TPL_DOMAIN_LANG),
				'expert'       => __( 'Expert', TPL_DOMAIN_LANG),
			)
		);
...
//var_dump($children);
	...
	/** NhatHuy - Course type */
	/**
	 * @param null $course_type
	 *
	 * @return mixed
	 *
	 * Get the users / students / course types
	 *
	 * @since v.1.0.0
	 */
	public function course_types( $course_type = null ) {
		$course_types = apply_filters(
			'tutor_course_type',
			array(
				'forinternal'     => __( 'For internal', TPL_DOMAIN_LANG),
				'forexternal' => __( 'For external', TPL_DOMAIN_LANG),
			)
		);

		if ( $course_type ) {
			if ( isset( $course_types[ $course_type ] ) ) {
...
//NhatHuy	
	...
	public function get_total_courses_children_topics( $course_id ) {
		...
		$children_ids_arr = explode(" ",$children_ids);
		$count_total = 0;
		foreach($children_ids_arr as $c_c){
			$obj_name = $this->get_topics( $c_c );
			$count_total +=  $obj_name->post_count;
		}

		return $count_total;
	}
	
	public function get_total_courses_children_lessons( $course_id ) {
		global $wpdb;
		...
		$count_total = 0;
		foreach($children_ids_arr as $c_c){
			$count_total += count( $this->get_course_content_ids_by( tutor()->lesson_post_type, tutor()->course_post_type, $c_c ) );
		}
...
//var_dump($attempt);die();
	...
	public function course_progress_status_context( $course_id = 0, $user_id = 0 ) {
		...
		$html = '';
		if ( $is_completed ) {
			$html = '<span class="course-completion-status course-completed"><i class="tutor-icon-mark-filled"></i> ' . __( 'Completed', TPL_DOMAIN_LANG) . ' </span>';
		} else {
			$is_in_progress = tutor_utils()->get_completed_lesson_count_by_course( $course_id, $user_id );
			if ( $is_in_progress ) {
				$html = '<span class="course-completion-status course-inprogress"><i class="tutor-icon-refresh-l"></i> ' . __( 'In Progress', TPL_DOMAIN_LANG) . ' </span>';
			} else {
				$html = '<span class="course-completion-status course-not-taken"><i class="tutor-icon-spinner"></i> ' . __( 'Not Taken', TPL_DOMAIN_LANG) . ' </span>';
			}
		}
		return $html;
	}
	...
	public function tutor_pages() {
		$pages = apply_filters(
			'tutor_pages',
			array(
				'tutor_dashboard_page_id'  => __( 'Dashboard Page', TPL_DOMAIN_LANG),
				'instructor_register_page' => __( 'Instructor Registration Page', TPL_DOMAIN_LANG),
				'student_register_page'    => __( 'Student Registration Page', TPL_DOMAIN_LANG),
			)
		);
	...
	public function user_profile_completion( $user_id = 0 ) {
		...
		$required_fields = array(
			// 'first_name' 				  => sprintf(__('Set Your %sFirst Name%s', TPL_DOMAIN_LANG), '<a class="tutor-color-black" href="' . $settings_url . '">', '</a>'),
			// 'last_name' 				  => sprintf(__('Set Your %sLast Name%s', TPL_DOMAIN_LANG), '<a class="tutor-color-black" href="' . $settings_url . '">', '</a>'),
			'_tutor_profile_photo' => sprintf( __( 'Set Your %1$sProfile Photo%2$s', TPL_DOMAIN_LANG), '<a class="tutor-color-black" href="' . $settings_url . '">', '</a>' ),
			'_tutor_profile_bio'   => sprintf( __( 'Set Your %1$sBio%2$s', TPL_DOMAIN_LANG), '<a class="tutor-color-black" href="' . $settings_url . '">', '</a>' ),
		);

		// Add payment method as a required on if current user is an approved instructor
		if ( 'approved' == $instructor_status ) {
			$required_fields['_tutor_withdraw_method_data'] = sprintf( __( 'Set %1$sWithdraw Method%2$s', TPL_DOMAIN_LANG), '<a class="tutor-color-black" href="' . $withdraw_settings_url . '">', '</a>' );
		}
	...
	*/
	...
	public function tutor_empty_state( string $title = 'No data yet!' ) {
		...
		<div class="tutor-empty-state td-empty-state tutor-p-32 tutor-text-center">
			...
			<div class="tutor-fs-6 tutor-fw-normal tutor-color-black-60 tutor-text-center">
				<?php echo sprintf( esc_html_x( '%s', $page_title, TPL_DOMAIN_LANG), $page_title ); ?>
			</div>
		</div>
		<?php
	}
	...
	public function translate_dynamic_text( $key, $add_badge = false, $badge_tag = 'span' ): string {
		...
		$key_value = array(
			'pending'    => array(
				'badge' => 'warning',
				'text'  => __( 'Pending', TPL_DOMAIN_LANG),
			),
			'pass'       => array(
				'badge' => 'success',
				'text'  => __( 'Pass', TPL_DOMAIN_LANG),
			),
			'correct'    => array(
				'badge' => 'success',
				'text'  => __( 'Correct', TPL_DOMAIN_LANG),
			),
			'fail'       => array(
				'badge' => 'danger',
				'text'  => __( 'Fail', TPL_DOMAIN_LANG),
			),
			'wrong'      => array(
				'badge' => 'danger',
				'text'  => __( 'Wrong', TPL_DOMAIN_LANG),
			),
			'approved'   => array(
				'badge' => 'success',
				'text'  => __( 'Approved', TPL_DOMAIN_LANG),
			),
			'rejected'   => array(
				'badge' => 'danger',
				'text'  => __( 'Rejected', TPL_DOMAIN_LANG),
			),
			'completed'  => array(
				'badge' => 'success',
				'text'  => __( 'Completed', TPL_DOMAIN_LANG),
			),
			'processing' => array(
				'badge' => 'warning',
				'text'  => __( 'Processing', TPL_DOMAIN_LANG),
			),
			'cancelled'  => array(
				'badge' => 'danger',
				'text'  => __( 'Cancelled', TPL_DOMAIN_LANG),
			),
			'canceled'   => array(
				'badge' => 'danger',
				'text'  => __( 'Cancelled', TPL_DOMAIN_LANG),
			...
			'blocked'    => array(
				'badge' => 'danger',
				'text'  => __( 'Blocked', TPL_DOMAIN_LANG),
			),
			'cancel'     => array(
				'badge' => 'danger',
				'text'  => __( 'Cancelled', TPL_DOMAIN_LANG),
			...
			'on-hold'    => array(
				'badge' => 'warning',
				'text'  => __( 'On Hold', TPL_DOMAIN_LANG),
			),
			'onhold'     => array(
				'badge' => 'warning',
				'text'  => __( 'On Hold', TPL_DOMAIN_LANG),
			...
			'publish'    => array(
				'badge' => 'success',
				'text'  => __( 'Publish', TPL_DOMAIN_LANG),
			),
			'trash'      => array(
				'badge' => 'danger',
				'text'  => __( 'Trash', TPL_DOMAIN_LANG),
			),
			'draft'      => array(
				'badge' => 'warning',
				'text'  => __( 'Draft', TPL_DOMAIN_LANG),
			),
			'private'    => array(
				'badge' => 'warning',
				'text'  => __( 'Private', TPL_DOMAIN_LANG),
			),
		);
	...
	public function course_nav_items() {
		$array = array(
			'info'          => array(
				'title'  => __( 'Course Info', TPL_DOMAIN_LANG),
				'method' => 'tutor_course_info_tab',
			),
			'curriculum'    => array(
				'title'  => __( 'Curriculum', TPL_DOMAIN_LANG),
				'method' => 'tutor_course_topics',
			),
			'reviews'       => array(
				'title'  => __( 'Reviews', TPL_DOMAIN_LANG),
				'method' => 'tutor_course_target_reviews_html',
			),
			'questions'     => array(
				'title'             => __( 'Q&A', TPL_DOMAIN_LANG),
				'method'            => 'tutor_course_question_and_answer',
				'require_enrolment' => true,
			),
			'announcements' => array(
				'title'             => __( 'Announcements', TPL_DOMAIN_LANG),
	...
	public function get_quiz_attempt_timing( $attempt_data ) {
		...
		if ( is_array( $attempt_info ) ) {
			...
			if ( isset( $attempt_info['time_limit'] ) ) {
				$attempt_duration = $attempt_info['time_limit']['time_value'] . ' ' . __( ucwords( $attempt_info['time_limit']['time_type'] ), TPL_DOMAIN_LANG);
			}
	...
	public function not_found_text(): string {
		...
		$text     = array(
			'normal' => __( 'No Data Available in this Section', TPL_DOMAIN_LANG ),
	...
	public function instructor_menus(): array {
		return array(
			'separator-1'   => array(
				'title'    => __( 'Instructor', TPL_DOMAIN_LANG),
				'show_ui'  => true,
				'auth_cap' => tutor()->instructor_role,
				'type'     => 'separator',
			),
			'my-courses'    => array(
				'title'    => __( 'All Courses', TPL_DOMAIN_LANG),
				'auth_cap' => tutor()->instructor_role,
	...
	public function admin_allcourses_menus(): array {
		return array(
			...
			'enroll-students'  => array(
				'title' => __( 'Enroll Students', TPL_DOMAIN_LANG),
				'auth_cap' => 'administrator',
				'icon'  => 'tutor-icon-college-graduation-filled',
			),
		);
	...
	public function staff_menus(): array {
		return array(
			'separator-2'   => array(
				'title'    => __( 'Staff', TPL_DOMAIN_LANG),
				'auth_cap' => 'shop_manager',
				'type'     => 'separator',
			),
			'learning-analytics' => array(
				'title'    => __( 'Learning Analytics', TPL_DOMAIN_LANG),
				'show_ui'  => true,
				'auth_cap' => 'shop_manager',
				'icon'     => 'tutor-icon-report-filled',
			),
			'learning-report' => array(
				'title'    => __( 'Learning Report', TPL_DOMAIN_LANG),
				'show_ui'  => false,
				'auth_cap' => 'shop_manager',
				'icon'     => 'tutor-icon-report-filled',
			),
	...
	public function stlt_menus(): array {
		return array(
			'separator-2'   => array(
				'title'    => __( 'Management', TPL_DOMAIN_LANG),
				'auth_cap' => 'st_lt',
				'type'     => 'separator',
			),
			'my-courses'    => array(
	...
	public function analytics_menus(): array {
		return array(
			'separator-2'   => array(
				'title'    => __( 'Analytics', TPL_DOMAIN_LANG),
				'auth_cap' => 'administrator',
				'type'     => 'separator',
			),
			'learning-analytics' => array(
	...
	public function default_menus(): array {
		return array(
			...
			'my-profile'       => array(
				'title' => __( 'My Profile', TPL_DOMAIN_LANG),
			...
			'enrolled-courses' => array(
				'title' => __( 'Enrolled  Courses', TPL_DOMAIN_LANG),
				'show_ui'  => false,
				'icon'  => 'tutor-icon-college-graduation-filled',
			), 
			'wishlist'         => array(
				'title' => __( 'Wishlist', TPL_DOMAIN_LANG),
				'show_ui'  => false,
			...
			'my-quiz-attempts' => array(
				'title' => __( 'My Quiz Attempts', TPL_DOMAIN_LANG),
				'show_ui'  => false,
				'icon'  => 'tutor-icon-quiz-attempt-filled',
			),
			'purchase_history' => array(
				'title' => __( 'Order History', TPL_DOMAIN_LANG),
				'show_ui'  => false,
				'icon'  => 'tutor-icon-cart-filled',
			),
			'question-answer'  => array(
				'title' => __( 'Question & Answer', TPL_DOMAIN_LANG),
			...
			// ),			
			'create-course'  => array(
				'title' => __( 'create course', TPL_DOMAIN_LANG),
				'icon'  => 'tutor-icon-college-graduation-filled',
				'show_ui'  => false,
			),
		);
	...
	public function get_video_sources( bool $key_title_only ) {
		...
		$video_sources = array(
			'html5'        => array(
				'title' => __( 'HTML 5 (mp4)', TPL_DOMAIN_LANG),
				'icon'  => 'html5',
			),
			'external_url' => array(
				'title' => __( 'External URL', TPL_DOMAIN_LANG),
				'icon'  => 'external_url',
			),
			'youtube'      => array(
				'title' => __( 'Youtube', TPL_DOMAIN_LANG),
				'icon'  => 'youtube',
			),
			'vimeo'        => array(
				'title' => __( 'Vimeo', TPL_DOMAIN_LANG),
				'icon'  => 'vimeo',
			),
			'embedded'     => array(
				'title' => __( 'Embedded', TPL_DOMAIN_LANG),
				'icon'  => 'embedded',
			),
			'shortcode'    => array(
				'title' => __( 'Shortcode', TPL_DOMAIN_LANG),
				'icon'  => 'code',
			),
		);
