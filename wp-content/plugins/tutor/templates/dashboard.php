		...
		<div class="tutor-row tutor-d-flex tutor-justify-content-between tutor-frontend-dashboard-header">
					...
					?>
						<div class="tutor-dashboard-header-display-name tutor-color-black">
							<div class="tutor-fs-5 tutor-fw-normal tutor-dashboard-header-greetings">
								<?php echo _e("Hello", TPL_DOMAIN_LANG) ?>,
							</div>
							<div class="tutor-fs-4 tutor-fw-medium tutor-dashboard-header-username">
								<?php echo esc_html($user->display_name); ?>
							</div>
			...
			<div class="tutor-header-right-side tutor-col-md-6 tutor-d-flex tutor-justify-content-end tutor-mt-20 tutor-mt-md-0">
				<div class="tutor-d-flex tutor-align-items-center">
					...
					//echo SYSTEM_KEY;
					if('SYSTEM_KEY' == "__VUS_TRAININGHUB_HIDE") {
						ob_start();
						...
						?>
							<a id="tutor-become-instructor-button" style="vertical-align:middle" class="tutor-btn tutor-btn-tertiary tutor-is-outline" href="<?php echo esc_url(tutor_utils()->instructor_register_url()); ?>">
								<i class="tutor-icon-man-user-filled tutor-icon-24"></i> &nbsp; <?php _e('Become an instructor', TPL_DOMAIN_LANG); ?>
							</a>
						<?php
						}
					...
					//if (current_user_can(tutor()->instructor_role)) {
					if (current_user_can( 'administrator' ) || current_user_can( 'st_lt' )) {
					...
					?>
						<a class="tutor-btn tutor-btn-tertiary tutor-is-outline" href="<?php echo esc_url(apply_filters('frontend_course_create_url', admin_url('post-new.php?post_type=' . tutor()->course_post_type))); ?>">
							<i class="tutor-icon-plus-square-filled tutor-icon-24 tutor-mr-12"></i> <?php esc_html_e('Create a New Course', TPL_DOMAIN_LANG); ?>
						</a>
					<?php
						...
						__('Your Application is pending as of', TPL_DOMAIN_LANG), ' <b>', $on, '</b>',
						'</span>';
			...
			<div class="tutor-col-12 tutor-col-md-4 tutor-col-lg-3 tutor-dashboard-left-menu">
				<ul class="tutor-dashboard-permalinks">
					...
					foreach ($dashboard_pages as $dashboard_key => $dashboard_page) {
						...
						if( trim($menu_title) != "Thông tin cá nhân")  {
							...
							if ($separator) {
								...
								if ($menu_title) {
									echo "<li class='tutor-dashboard-menu-divider-header'>"._e($menu_title,TPL_DOMAIN_LANG)."</li>";
								}
							} else {
								$li_class = "tutor-dashboard-menu-{$dashboard_key} _menu_li_item";
								if ($dashboard_key === 'index') {
									$dashboard_key = '';
								}
								$active_class    = $dashboard_key == $dashboard_page_slug ? 'active' : '';
								$data_no_instant = 'logout' == $dashboard_key ? 'data-no-instant' : '';
								if($menu_title != "My Profile"){
									echo "<li class='tutor-dashboard-menu-item {$li_class}  {$active_class}'><a {$data_no_instant} href='" . $menu_link . "' class='tutor-dashboard-menu-item-link tutor-fs-6 tutor-fw-normal tutor-color-black'>{$menu_icon} <span class='tutor-dashboard-menu-item-text tutor-ml-12'>";
									echo _e($menu_title,TPL_DOMAIN_LANG);
									echo "</span></a></li>";
								}
