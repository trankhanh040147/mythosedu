...
class REST_Course  {
	...
	*/
	public function enroll_course(WP_REST_Request $request) {
		...
		$us = $headers['username'][0];
		$pw = $headers['password'][0];
		
		$userdata = get_user_by('login', $us);
		if(empty($userdata)) {
			$response = array(
				'status_code'=> "Not Authorized",
				"message"=>  __("Invalid username",'tutor'),
				'data'=> []
			);
			return self::send($response);
		}
		
		$resultpass = wp_check_password($pw, $userdata->user_pass, $userdata->ID);
		
		if(!$resultpass) {
			$response = array(
				'status_code'=> "Not Authorized",
				"message"=>  __("Invalid password",'tutor'),
				'data'=> []
			);
			return self::send($response);
		}//var_dump((array) $userdata->roles);
		if ( !in_array( 'administrator', (array) $userdata->roles ) && !in_array( 'shop_manager', (array) $userdata->roles ) ) {
			$response = array(
				'status_code'=> "Not Authorized",
				"message"=>  __("1Invalid role",'tutor'),
				'data'=> []
			);
			return self::send($response);
		}
		 $course_id			= sanitize_text_field($request->get_param('course_id'));
		$tqm_course_code    = sanitize_text_field($request->get_param('tqm_course_code'));
		
		if(!$course_id && !$tqm_course_code) {
			$response = array(
				'status_code'=> "validation_error",
				"message"=>  __("Please input valid course_id or tqm_course_code",'tutor'),
				'data'=> []
			);
			return self::send($response);
		}
		...
		//var_dump($students);
		foreach ($students_arr as $subarray) {
			if ($subarray) {
				$result[] = implode(',', $subarray);
			}
		}
		$students = implode(';', $result);
		if( !$students){
			$response = array(
				'status_code'=> "validation_error",
				"message"=>  __("Please enter valid students list",'tutor'),
				'data'=> []
			);	
			return self::send($response);
		}
		//die();
		$staff_id = 0;	
		if(in_array( 'shop_manager', (array) $userdata->roles ))
			$staff_id = $userdata->ID;
		if(tutor_utils()->do_enroll_api($course_id,$tqm_course_code,$students,$staff_id)){
			$response = array(
				'status_code'=> "enroll_success",
				"message"=>  __("Course enrolled",'tutor'),
				'data'=> []
			);	
			return self::send($response);
		}
		...
		$response = array(
			'status_code'=> "enroll_error",
			"message"=>  __("error",'tutor'),
			'data'=> []
		);	
		return self::send($response);		
	}
