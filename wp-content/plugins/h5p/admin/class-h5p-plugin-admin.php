...
class H5P_Plugin_Admin {
  ...
  private function __construct() {
    ...
    //nhathuy// add_action('media_buttons', array($this->content, 'add_insert_button'));
  ...
  public function add_plugin_admin_menu() {
    ...
    add_menu_page($h5p_content, $h5p_content, 'view_h5p_contents', $this->plugin_slug, array($this->content, 'display_contents_page'), 'none', 3);
...
//      $site_key = get_option('h5p_site_key', get_option('h5p_h5p_site_uuid', FALSE));
  ...
  public function ajax_results() {
	...
	if (!$GPA) {
      H5PCore::ajaxError(__('Not found Grade Point Average', $this->plugin_slug));
      exit;
    }
	...
	if($h5p_points_answered_all && $quiz_points_answered_all && !$is_completed_course){
		...
		do {
						...
						$hasHash = (int) $wpdb->get_var(
							$wpdb->prepare(
								"SELECT COUNT(comment_ID) from {$table_comments}
		...
		$data = array(
						...
						'comment_content'  => '', // Identification Hash
		...
		$insert_finish = $wpdb->insert( $table_comments, $data );
