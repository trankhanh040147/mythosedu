...
H5P.setFinished = function (contentId, score, maxScore, time) {
  ...
  if (validScore && H5PIntegration.postUserStatistics === true) {
    ...
    H5P.jQuery.post(H5PIntegration.ajax.setFinished, data)
	  ...
	  window.parent.H5P.jQuery('.h5pleave').removeClass('h5p-leave');
        if(data.data.GPA){
			H5P.jQuery( "div.feedback-text" ).last().append("<div style='padding: 0 10px;color:blue;display: -webkit-inline-flex;'>Passing Grade: "+data.data.GPA+"%</div>");
		}
		if(data.data.earned_percentage){
			H5P.jQuery( "div.feedback-text" ).last().append("<div style='padding: 0 10px;color:red;display: -webkit-inline-flex;'>Total Earned: "+data.data.earned_percentage+"%</div>");
		}
		if(data.data.completed){
			H5P.jQuery( "div.feedback-text" ).last().append("<div style='padding: 0 10px;color:green;display: -webkit-inline-flex;'>Course completed</div>");
		}
		if(data.data.lesson_url){
			H5P.jQuery( "div.buttons" ).last().append("<a href='"+data.data.lesson_url+"' class='h5p-joubelui-button h5p-button'>Back To Course</a>");
		}
      })	
      .fail(function () {
