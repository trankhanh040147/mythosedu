...
class Lesson extends Tutor_Base
{
	...
	public function tutor_modal_create_or_update_lesson()
	{
		...
		$post_id_query = $wpdb->prepare("SELECT ID FROM tbl_posts WHERE post_parent = $topic_id");
		$get_post_id = $wpdb->get_results($post_id_query);

		$lesson_video_query = $wpdb->prepare("SELECT * FROM tbl_postmeta WHERE post_id = %d", $get_post_id[0]->ID);
		$lesson_video = $wpdb->get_results($lesson_video_query);

		if ($lesson_video[0]->meta_key == '_video') {
		...
		if (0 === $lesson_id) {
			...
			$tbl_lesson = array(
				...
				'lesson_title' => $title,
				'lesson_video' => $video,
				'lesson_decription' => $post_content,
			...
			$update_lesson_data = array(
				...
				'lesson_video' => $video,
				'lesson_decription' => $lesson_data['post_content'],
				'lesson_status' => 'publish',
				'data_meta' => 'data_meta',
			);
	...
	public function tutor_delete_lesson_by_id()
	{
		...
		global $wpdb;
		$sql = "DELETE FROM tbl_lesson WHERE ID = $lesson_id";
		$wpdb->query($sql);
	...
	public function reply_lesson_comment()
	{
		...
		ob_start();
		?>
		<div class="tutor-comments-list tutor-child-comment tutor-mt-32"
			id="lesson-comment-<?php echo esc_attr($reply->comment_ID); ?>">
