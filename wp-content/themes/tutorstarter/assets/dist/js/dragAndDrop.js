function factory($) {
  ...
  function Sortable(el, options) {
    var self = this,
      ...
      node_type =
        container_type == "OL" || container_type == "UL" ? "LI" : "DIV",
      defaults = {
        //options
        handle: false,
        ...
        dragstart: function (evt) {
          ...
          if (self.options.autocreate) {
            self
              ...
              .append(
                "<" +
                self.options.container_type +
                ' class="' +
                self.options.auto_container_class +
                '"/>'
              );
          }
        },
  ...
  Draggable.prototype.init = function () {
    ...
    function check_droptarget(pos) {
      ...
      $clone.hide();
      $over = $(document.elementFromPoint(pos.clientX, pos.clientY)).closest(
        self.options.droptarget
      );
      $clone.show();
  ...
  function Dragaware(el, options) {
    ...
    function autoscroll(pos) {
      ...
      if (scrollLeft > 0 && mouse.x < offset.left) {
        ...
        scrollLeft < sp.prop("scrollWidth") - width &&
        ...
        scrollTop < sp.prop("scrollHeight") - height &&
    ...
    function start(evt) {
      ...
      $reference = options.delegate
        ? $target.closest(options.delegate)
        : $dragaware;

      if (
        $target.closest(options.handle || "*").length &&
        (evt.type == "touchstart" || evt.button == 0)
      ) {
    ...
    return this.parents()
      ...
      .filter(function () {
        ...
        return /(scroll|auto)/.test(
          p.css("overflow-x") + p.css("overflow-y") + p.css("overflow")
        );
      });
  };
